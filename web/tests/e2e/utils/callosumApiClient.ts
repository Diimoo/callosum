...
/**
 * API Client for Callosum backend operations in E2E tests.
 *
 * Provides a type-safe, abstracted interface for interacting with the Callosum backend API.
 ...
 */
export class CallosumApiClient {
  ...
  private async handleResponseSoft(
    ...
    if (!response.ok()) {
      const errorText = await response.text();
      console.error(
        `[CallosumApiClient] ${errorMessage}: ${response.status()} - ${errorText}`
      );
      return false;
    }
    return true;
   ...
   *
   * @param message - The message to log (will be prefixed with "[CallosumApiClient]")
   */
  private log(message: string): void {
    console.log(`[CallosumApiClient] ${message}`);
  }
