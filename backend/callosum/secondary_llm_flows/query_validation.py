...
from callosum.llm.utils import message_generator_to_string_generator
from callosum.llm.utils import message_to_string
from callosum.prompts.constants import ANSWERABLE_PAT
from callosum.prompts.constants import THOUGHT_PAT
from callosum.prompts.query_validation import ANSWERABLE_PROMPT
from callosum.server.query_and_chat.models import QueryValidationResponse
    ...
    try:
        ...
        for token in tokens:
            ...
            if not reasoning_pat_found and THOUGHT_PAT.upper() in model_output:
                ...
                if remaining:
                    yield get_json_line(
                        CallosumAnswerPiece(answer_piece=remaining).model_dump()
                    )
                continue
            ...
            if reasoning_pat_found:
                ...
                if hold_answerable == ANSWERABLE_PAT.upper()[: len(hold_answerable)]:
                    continue
                yield get_json_line(
                    CallosumAnswerPiece(answer_piece=hold_answerable).model_dump()
                )
                hold_answerable = ""
