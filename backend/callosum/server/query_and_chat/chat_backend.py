...
from callosum.chat.prompt_builder.citations_prompt import (
...
from callosum.llm.factory import get_default_llms
...
async def speech_to_text(
    ...
    try:
        response = requests.post(
            f"{model_server_url}/speech/whisperx",
            files={
                "file": (
        ...
        logger.exception("Error calling WhisperX STT model server")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to run speech-to-text: {e}",
        )
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Invalid STT response from model server")
