...
from callosum.configs.constants import DocumentIndexType
from callosum.configs.constants import QueryHistoryType
from callosum.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from callosum.prompts.image_analysis import DEFAULT_IMAGE_ANALYSIS_SYSTEM_PROMPT
from callosum.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_SYSTEM_PROMPT
from callosum.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_USER_PROMPT
...
# 3. no queries
CALLOSUM_QUERY_HISTORY_TYPE = QueryHistoryType(
    (os.environ.get("CALLOSUM_QUERY_HISTORY_TYPE") or QueryHistoryType.NORMAL.value).lower()
)
...
# NOTE: if you are having problems accessing the Callosum web UI locally (especially
...
# and is available in Callosum EE
...
# restrict access to Callosum to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict Callosum
...
SAML_CONF_DIR = os.environ.get("SAML_CONF_DIR") or "/app/callosum/configs/saml_config"
...
# If set, Callosum will listen to the `expires_at` returned by the identity
...
VESPA_DEPLOYMENT_ZIP = (
    os.environ.get("VESPA_DEPLOYMENT_ZIP") or "/app/callosum/vespa-app.zip"
)
...
# View the list here:
# https://github.com/callosum-dot-app/callosum/blob/main/backend/callosum/connectors/factory.py
...
CONFLUENCE_USE_CALLOSUM_USERS_FOR_GROUP_SYNC = (
    os.environ.get("CONFLUENCE_USE_CALLOSUM_USERS_FOR_GROUP_SYNC", "").lower() == "true"
)
...
# backend/callosum/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja.
...
# Logs Callosum only model interactions like prompts, responses, messages etc.
LOG_CALLOSUM_MODEL_INTERACTIONS = (
    os.environ.get("LOG_CALLOSUM_MODEL_INTERACTIONS", "").lower() == "true"
)
...
BRAINTRUST_PROJECT = os.environ.get("BRAINTRUST_PROJECT", "Callosum")
...
# founders@callosum.app OR message Chris Weaver or Yuhong Sun in the Callosum
...
S3_FILE_STORE_BUCKET_NAME = (
    os.environ.get("S3_FILE_STORE_BUCKET_NAME") or "callosum-file-store-bucket"
)
S3_FILE_STORE_PREFIX = os.environ.get("S3_FILE_STORE_PREFIX") or "callosum-files"
