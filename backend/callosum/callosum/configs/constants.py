...
CALLOSUM_DEFAULT_APPLICATION_NAME = "Callosum"
CALLOSUM_DISCORD_URL = "https://discord.gg/4NA5SbzrWb"
SLACK_USER_TOKEN_PREFIX = "xoxp-"
SLACK_BOT_TOKEN_PREFIX = "xoxb-"
CALLOSUM_EMAILABLE_LOGO_MAX_DIM = 512
...
TENANT_ID_COOKIE_NAME = "callosum_tid"  # tenant id - for workaround cases
ANONYMOUS_USER_COOKIE_NAME = "callosum_anonymous_user"

NO_AUTH_USER_ID = "__no_auth_user__"
NO_AUTH_USER_EMAIL = "anonymous@callosum.app"
...
CALLOSUM_METADATA_FILENAME = ".callosum_metadata.json"

# Messages
DISABLED_GEN_AI_MSG = (
    "Your System Admin has disabled the Generative AI functionalities of Callosum.\n"
    "Please contact them if you wish to have this enabled.\n"
    "You can still use Callosum as a search engine."
)
...
DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN = "callosumapikey.ai"
...
KV_SETTINGS_KEY = "callosum_settings"
KV_CUSTOMER_UUID_KEY = "customer_uuid"
KV_INSTANCE_DOMAIN_KEY = "instance_domain"
KV_ENTERPRISE_SETTINGS_KEY = "callosum_enterprise_settings"
...
class DocumentSource(str, Enum):
    # Special case, document passed in via Callosum APIs without specifying a source type
...
class MilestoneRecordType(str, Enum):
    ...
    CREATED_CALLOSUM_BOT = "created_callosum_bot"
...
class CallosumRedisConstants:
    ACTIVE_FENCES = "active_fences"


class CallosumCeleryPriority(int, Enum):
    HIGHEST = 0
    HIGH = auto()
    MEDIUM = auto()
    LOW = auto()
...
CALLOSUM_CLOUD_CELERY_TASK_PREFIX = "cloud"

# the tenant id we use for system level redis operations
CALLOSUM_CLOUD_TENANT_ID = "cloud"

# the redis namespace for runtime variables
CALLOSUM_CLOUD_REDIS_RUNTIME = "runtime"
CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT = 600


class CallosumCeleryTask:
    DEFAULT = "celery"

    CLOUD_BEAT_TASK_GENERATOR = f"{CALLOSUM_CLOUD_CELERY_TASK_PREFIX}_generate_beat_tasks"
    CLOUD_MONITOR_ALEMBIC = f"{CALLOSUM_CLOUD_CELERY_TASK_PREFIX}_monitor_alembic"
    CLOUD_MONITOR_CELERY_QUEUES = (
        f"{CALLOSUM_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_queues"
    )
    CLOUD_CHECK_AVAILABLE_TENANTS = (
        f"{CALLOSUM_CLOUD_CELERY_TASK_PREFIX}_check_available_tenants"
    )
    CLOUD_MONITOR_CELERY_PIDBOX = (
        f"{CALLOSUM_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_pidbox"
    )
...
CALLOSUM_CELERY_BEAT_HEARTBEAT_KEY = "callosum:celery:beat:heartbeat"
...
class CallosumCallTypes(str, Enum):
    FIREFLIES = "FIREFLIES"
    GONG = "GONG"
