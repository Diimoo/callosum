...
from callosum.configs.app_configs import DISABLE_TELEMETRY
from callosum.configs.constants import ID_SEPARATOR
from callosum.configs.constants import MessageType
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_FEEDBACK_VISIBILITY
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_MAX_QPM
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_MAX_WAIT_TIME
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_NUM_RETRIES
from callosum.configs.callosumbot_configs import (
    CALLOSUM_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
)
from callosum.configs.callosumbot_configs import (
    CALLOSUM_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
)
from callosum.connectors.slack.utils import SlackTextCleaner
...
from callosum.callosumbot.slack.constants import FeedbackVisibility
from callosum.callosumbot.slack.models import ChannelType
from callosum.callosumbot.slack.models import ThreadMessage
from callosum.prompts.miscellaneous_prompts import SLACK_LANGUAGE_REPHRASE_PROMPT
...
from callosum.utils.text_processing import replace_whitespaces_w_space
...
_CALLOSUM_BOT_MESSAGE_COUNT: int = 0
_CALLOSUM_BOT_COUNT_START_TIME: float = time.time()


def get_callosum_bot_auth_ids(
...
def check_message_limit() -> bool:
    ...
    """
    if CALLOSUM_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD <= 0:
        return True
    global _CALLOSUM_BOT_MESSAGE_COUNT
    global _CALLOSUM_BOT_COUNT_START_TIME
    time_since_start = time.time() - _CALLOSUM_BOT_COUNT_START_TIME
    if time_since_start > CALLOSUM_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS:
        _CALLOSUM_BOT_MESSAGE_COUNT = 0
        _CALLOSUM_BOT_COUNT_START_TIME = time.time()
    if (_CALLOSUM_BOT_MESSAGE_COUNT + 1) > CALLOSUM_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            f"CallosumBot has reached the message limit {CALLOSUM_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD}"
            f" for the time period {CALLOSUM_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS} seconds."
            " These limits are configurable in backend/callosum/configs/callosumbot_configs.py"
        )
        return False
    _CALLOSUM_BOT_MESSAGE_COUNT += 1
    return True
...
def remove_callosum_bot_tag(tenant_id: str, message_str: str, client: WebClient) -> str:
    bot_token_user_id, _ = get_callosum_bot_auth_ids(tenant_id, web_client=client)
...
def _build_error_block(error_message: str) -> Block:
    ...
    """
    display_text = (
        "There was an error displaying all of the Callosum answers."
        f" Please let an admin or an callosum developer know. Error: {error_message}"
    )
    return SectionBlock(text=display_text)


@retry(
    tries=CALLOSUM_BOT_NUM_RETRIES,
    delay=0.25,
    backoff=2,
...
def read_slack_thread(
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            blocks: Any
            is_callosum_bot_response = False

            reply_user = reply.get("user")
            reply_bot_id = reply.get("bot_id")

            self_slack_bot_user_id, self_slack_bot_bot_id = get_callosum_bot_auth_ids(
                tenant_id, client
            )
            if reply_user is not None and reply_user == self_slack_bot_user_id:
                is_callosum_bot_response = True

            if reply_bot_id is not None and reply_bot_id == self_slack_bot_bot_id:
                is_callosum_bot_response = True

            if is_callosum_bot_response:
                # CallosumBot response
                message_type = MessageType.ASSISTANT
                user_sem_id = "Assistant"

                # CallosumBot responses have both text and blocks
                ...
                if not blocks:
                    logger.warning(f"CallosumBot response has no blocks: {reply}")
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the CallosumBot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from Callosum
                message_type = MessageType.USER
        ...
        message = remove_callosum_bot_tag(tenant_id, message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            callosum_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(callosum_user.id) if callosum_user else "Non-Callosum-Or-No-Auth-User",
    )


class SlackRateLimiter:
    def __init__(self) -> None:
        self.max_qpm: int | None = CALLOSUM_BOT_MAX_QPM
        self.max_wait_time = CALLOSUM_BOT_MAX_WAIT_TIME
...
def get_feedback_visibility() -> FeedbackVisibility:
    try:
        return FeedbackVisibility(CALLOSUM_BOT_FEEDBACK_VISIBILITY.lower())
    except ValueError:
