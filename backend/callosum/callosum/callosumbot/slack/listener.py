...
from callosum.configs.app_configs import DEV_MODE
from callosum.configs.app_configs import POD_NAME
from callosum.configs.app_configs import POD_NAMESPACE
from callosum.configs.constants import MessageType
from callosum.configs.constants import CallosumRedisLocks
from callosum.configs.callosumbot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_REPHRASE_MESSAGE
from callosum.connectors.slack.utils import expert_info_from_slack_id
from callosum.context.search.retrieval.search_runner import (
    download_nltk_data,
)
...
from callosum.db.slack_bot import fetch_slack_bot
from callosum.db.slack_bot import fetch_slack_bots
...
from callosum.callosumbot.slack.config import get_slack_channel_config_for_bot_and_channel
from callosum.callosumbot.slack.config import MAX_TENANTS_PER_POD
from callosum.callosumbot.slack.config import TENANT_ACQUISITION_INTERVAL
from callosum.callosumbot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from callosum.callosumbot.slack.config import TENANT_HEARTBEAT_INTERVAL
from callosum.callosumbot.slack.config import TENANT_LOCK_EXPIRATION
from callosum.callosumbot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from callosum.callosumbot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from callosum.callosumbot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from callosum.callosumbot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from callosum.callosumbot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from callosum.callosumbot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from callosum.callosumbot.slack.constants import KEEP_TO_YOURSELF_ACTION_ID
from callosum.callosumbot.slack.constants import LIKE_BLOCK_ACTION_ID
from callosum.callosumbot.slack.constants import SHOW_EVERYONE_ACTION_ID
from callosum.callosumbot.slack.constants import VIEW_DOC_FEEDBACK_ID
from callosum.callosumbot.slack.handlers.handle_buttons import handle_doc_feedback_button
from callosum.callosumbot.slack.handlers.handle_buttons import handle_followup_button
from callosum.callosumbot.slack.handlers.handle_buttons import (
...
from callosum.callosumbot.slack.handlers.handle_buttons import handle_slack_feedback
from callosum.callosumbot.slack.handlers.handle_message import handle_message
from callosum.callosumbot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from callosum.callosumbot.slack.handlers.handle_message import schedule_feedback_reminder
from callosum.callosumbot.slack.models import SlackContext
from callosum.callosumbot.slack.models import SlackMessageInfo
from callosum.callosumbot.slack.utils import check_message_limit
from callosum.callosumbot.slack.utils import decompose_action_id
from callosum.callosumbot.slack.utils import get_channel_name_from_id
from callosum.callosumbot.slack.utils import get_channel_type_from_id
from callosum.callosumbot.slack.utils import get_callosum_bot_auth_ids
from callosum.callosumbot.slack.utils import read_slack_thread
from callosum.callosumbot.slack.utils import remove_callosum_bot_tag
from callosum.callosumbot.slack.utils import rephrase_slack_message
from callosum.callosumbot.slack.utils import respond_in_thread_or_channel
from callosum.callosumbot.slack.utils import TenantSocketModeClient
from callosum.redis.redis_pool import get_redis_client
from callosum.server.manage.models import SlackBotTokens
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        gated_tenants = fetch_ee_implementation_or_noop(
            "callosum.server.tenants.product_gating",
            "get_gated_tenants",
            set(),
        )()
        all_active_tenants = [
        ...
        for tenant_id in all_active_tenants:
            ...
            rlock: RedisLock = redis_client.lock(
                CallosumRedisLocks.SLACK_BOT_LOCK,
    ...
    def send_heartbeats(pod_id: str, tenant_ids: set[str]) -> None:
        ...
        for tenant_id in tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{CallosumRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{pod_id}"
            redis_client.set(
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_token_user_id, bot_token_bot_id = get_callosum_bot_auth_ids(
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_callosum_bot_tag(tenant_id, msg, client=client.web_client)
        ...
        if event_type == "message":
            ...
            if bot_token_user_id and bot_token_user_id in event_user:
                is_callosum_bot_msg = True

            if bot_token_bot_id and bot_token_bot_id in event_bot_id:
                is_callosum_bot_msg = True

            # CallosumBot should never respond to itself
            if is_callosum_bot_msg:
                logger.info("Ignoring message from CallosumBot (self-message)")
                return False

            # DMs with the bot don't pick up the @CallosumBot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If CallosumBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to Callosum or @CallosumBot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received CallosumBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to CallosumBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        bot_token_user_id, _ = get_callosum_bot_auth_ids(tenant_id, client.web_client)
        message_ts = event.get("ts")
        thread_ts = event.get("thread_ts")
        ...
        msg = remove_callosum_bot_tag(tenant_id, msg, client=client.web_client)

        if CALLOSUM_BOT_REPHRASE_MESSAGE:
        ...
        if tagged:
            logger.debug("User tagged CallosumBot")
...
def _get_socket_client(
    ...
    # https://docs.callosum.app/admins/getting_started/slack_bot_setup
