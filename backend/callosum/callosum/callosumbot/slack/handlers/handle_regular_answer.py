...
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_DISABLE_DOCS_ONLY_ANSWER
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_DISPLAY_ERROR_MSGS
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_NUM_RETRIES
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_REACT_EMOJI
...
from callosum.callosumbot.slack.handlers.utils import send_team_member_message
...
from callosum.callosumbot.slack.utils import SlackRateLimiter
...
def handle_regular_answer(
    ...
    num_retries: int = CALLOSUM_BOT_NUM_RETRIES,
    thread_context_percent: float = MAX_THREAD_CONTEXT_PERCENTAGE,
    should_respond_with_error_msgs: bool = CALLOSUM_BOT_DISPLAY_ERROR_MSGS,
    disable_docs_only_answer: bool = CALLOSUM_BOT_DISABLE_DOCS_ONLY_ANSWER,
) -> bool:
    ...
    # the message is a dm with the Callosum bot.
    send_as_ephemeral = (
    ...
    # or the message is a dm to the Callosum bot, we should use the proper callosum user from the email.
    # This will make documents privately accessible to the user available to Callosum Bot answers.
    # Otherwise - if not ephemeral or DM to Callosum Bot - we must use None as the user to restrict
    # to public docs.
    ...
    if not message_ts_to_respond_to and not is_slash_command:
        # if the message is not "/callosum" command, then it should have a message ts to respond to
        raise RuntimeError(
    ...
    try:
        ...
        answer = _get_slack_answer(
            ...
            callosum_user=user if can_search_over_private_docs else None,
        ...
        update_emote_react(
            emoji=CALLOSUM_BOT_REACT_EMOJI,
    ...
    if not answer.answer and disable_docs_only_answer:
        logger.notice(
            ...
            "`CALLOSUM_BOT_DISABLE_DOCS_ONLY_ANSWER` env variable is set"
        )
        return True
    ...
    try:
        ...
        # if there is no message_ts_to_respond_to, and we have made it this far, then this is a /callosum message
