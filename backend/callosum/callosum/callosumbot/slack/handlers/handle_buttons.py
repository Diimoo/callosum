...
from callosum.chat.process_message import remove_answer_citations
from callosum.configs.constants import MessageType
from callosum.configs.constants import SearchFeedbackType
from callosum.configs.callosumbot_configs import CALLOSUM_BOT_FOLLOWUP_EMOJI
...
from callosum.callosumbot.slack.blocks import build_follow_up_resolved_blocks
from callosum.callosumbot.slack.blocks import build_slack_response_blocks
from callosum.callosumbot.slack.blocks import get_document_feedback_blocks
...
from callosum.callosumbot.slack.handlers.handle_regular_answer import (
    handle_regular_answer,
)
from callosum.callosumbot.slack.models import SlackMessageInfo
from callosum.callosumbot.slack.utils import build_feedback_id
from callosum.callosumbot.slack.utils import decompose_action_id
from callosum.callosumbot.slack.utils import fetch_group_ids_from_names
from callosum.callosumbot.slack.utils import fetch_slack_user_ids_from_emails
from callosum.callosumbot.slack.utils import get_channel_name_from_id
from callosum.callosumbot.slack.utils import get_feedback_visibility
...
from callosum.callosumbot.slack.utils import update_emote_react
...
def handle_publish_ephemeral_message_button(
    ...
    # Obtain callosum_user and chat_message information
    if not chat_message_id:
        raise ValueError("Missing chat_message_id in the payload")

    with get_session_with_current_tenant() as db_session:
        callosum_user = get_user_by_email(user_email, db_session)
        if not callosum_user:
            raise ValueError("Cannot determine callosum_user_id from email in payload")
        try:
            chat_message = get_chat_message(chat_message_id, callosum_user.id, db_session)
        except ValueError:
        ...
        callosum_bot_answer = ChatBasicResponse(
    ...
    if action_id == SHOW_EVERYONE_ACTION_ID:
        ...
        try:
            ...
            respond_in_thread_or_channel(
                ...
                text="Hello! Callosum has some results for you!",
                blocks=all_blocks,
...
def handle_slack_feedback(
    ...
    # Get Callosum user from Slack ID
...
def handle_followup_button(
    ...
    update_emote_react(
        emoji=CALLOSUM_BOT_FOLLOWUP_EMOJI,
        channel=channel_id,
