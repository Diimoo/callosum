...
from callosum.file_processing.html_utils import ParsedHTML
...
DEFAULT_USER_AGENT = "CallosumWebCrawler/1.0 (+https://www.callosum.app)"


class CallosumWebCrawler(WebContentProvider):
    """
    ...
    def _fetch_url(self, url: str) -> WebContent:
        try:
            ...
            logger.warning(
                "Callosum crawler failed to fetch %s (%s)",
                url,
        ...
        if response.status_code >= 400:
            logger.warning("Callosum crawler received %s for %s", response.status_code, url)
            return WebContent(
                title="",
                link=url,
                full_content="",
        ...
        try:
            ...
            logger.warning(
                "Callosum crawler failed to parse %s (%s)", url, exc.__class__.__name__
            )
            text_content = ""
            title = ""
