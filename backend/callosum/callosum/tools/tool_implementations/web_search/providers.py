...
from callosum.db.models import InternetSearchProvider
...
from callosum.tools.tool_implementations.open_url.firecrawl import FirecrawlClient
from callosum.tools.tool_implementations.open_url.models import (
    WebContentProvider,
)
from callosum.tools.tool_implementations.open_url.callosum_web_crawler import CallosumWebCrawler
from callosum.tools.tool_implementations.web_search.clients.exa_client import (
    ExaClient,
)
from callosum.tools.tool_implementations.web_search.clients.google_pse_client import (
    GooglePSEClient,
)
from callosum.tools.tool_implementations.web_search.clients.searxng_client import (
    SearXNGClient,
)
from callosum.tools.tool_implementations.web_search.clients.serper_client import (
    SerperClient,
)
from callosum.tools.tool_implementations.web_search.models import DEFAULT_MAX_RESULTS
...
def build_content_provider_from_config(
    *,
    ...
    if provider_type == WebContentProviderType.CALLOSUM_WEB_CRAWLER:
        if config.timeout_seconds is not None:
            return CallosumWebCrawler(timeout_seconds=config.timeout_seconds)
        return CallosumWebCrawler()
