...
def prefix_user_email(user_email: str) -> str:
    ...
    This applies to both a Callosum user and an External user, this is to make the query time
    more efficient"""
...
def prefix_external_group(ext_group_name: str) -> str:
    """Prefixes an external group name to eliminate collision with user emails / Callosum groups."""
    return f"external_group:{ext_group_name}"


def build_ext_group_name_for_callosum(ext_group_name: str, source: DocumentSource) -> str:
    """
