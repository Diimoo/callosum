...
from callosum.access.utils import prefix_user_email
from callosum.configs.constants import DocumentSource
from callosum.configs.constants import PUBLIC_DOC_PAT
from callosum.db.document import get_access_info_for_document
from callosum.db.document import get_access_info_for_documents
...
def get_access_for_document(
    ...
    versioned_get_access_for_document_fn = fetch_versioned_implementation(
        "callosum.access.access", "_get_access_for_document"
    )
...
def get_access_for_documents(
    ...
    versioned_get_access_for_documents_fn = fetch_versioned_implementation(
        "callosum.access.access", "_get_access_for_documents"
    )
...
def get_acl_for_user(user: User | None, db_session: Session | None = None) -> set[str]:
    versioned_acl_for_user_fn = fetch_versioned_implementation(
        "callosum.access.access", "_get_acl_for_user"
    )
