...
from callosum.configs.constants import (
    DEFAULT_BOOST,
...
from callosum.configs.constants import TokenRateLimitScope
from callosum.connectors.models import InputType
from callosum.db.enums import ChatSessionSharedStatus
...
from callosum.db.pydantic_type import PydanticListType, PydanticType
...
from callosum.kg.models import KGStage
from callosum.server.features.mcp.models import MCPConnectionData
from callosum.utils.encryption import decrypt_bytes_to_string
from callosum.utils.encryption import encrypt_string_to_bytes
from callosum.utils.headers import HeaderItemDict
...
class ConnectorCredentialPair(Base):
    ...
    # {"customer_id": "123567", "company_domain": "@callosum.app"}
...
class Document(Base):
    ...
    # (as is passed around in Callosum)
...
class ChunkStats(Base):
    ...
    # this should correspond to the ID of the document
    # (as is passed around in Callosum)x
...
class ChatSession(Base):
    ...
    # This chat created by CallosumBot
    callosumbot_flow: Mapped[bool] = mapped_column(Boolean, default=False)
...
These models are only used in Enterprise Edition only features in Callosum.
They are kept here to simplify the codebase and avoid having different assumptions
on the shape of data being passed around between the MIT and EE versions of Callosum.

In the MIT version of Callosum, assume these tables are always empty.
"""
...
class User__ExternalUserGroupId(Base):
    ...
    directly adding the Callosum user to the doc ACL list"""
