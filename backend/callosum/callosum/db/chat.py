...
from callosum.chat.models import DocumentRelevance
...
from callosum.context.search.models import SearchDoc as ServerSearchDoc
from callosum.db.models import ChatMessage
from callosum.db.models import ChatMessage__SearchDoc
from callosum.db.models import ChatSession
from callosum.db.models import ChatSessionSharedStatus
...
from callosum.db.persona import get_best_persona_id_for_user
...
# Chat sessions do not include callosumbot flows
def get_chat_sessions_by_user(
    user_id: UUID | None,
    deleted: bool | None,
    db_session: Session,
    include_callosumbot_flows: bool = False,
    limit: int = 50,
    ...
    if not include_callosumbot_flows:
        stmt = stmt.where(ChatSession.callosumbot_flow.is_(False))
...
def create_chat_session(
    ...
    chat_session = ChatSession(
        ...
        callosumbot_flow=callosumbot_flow,
...
def delete_all_chat_sessions_for_user(
    ...
    chat_sessions = (
        ...
        .filter(ChatSession.user_id == user_id, ChatSession.callosumbot_flow.is_(False))
        .all()
    )
    ...
    if hard_delete:
        ...
        db_session.execute(
            delete(ChatSession).where(
                ChatSession.user_id == user_id, ChatSession.callosumbot_flow.is_(False)
            )
        )
    else:
        db_session.execute(
            update(ChatSession)
            .where(ChatSession.user_id == user_id, ChatSession.callosumbot_flow.is_(False))
            .values(deleted=True)
        )
...
def add_search_docs_to_tool_call(
    ...
    Args:
        ...
        db_session: The database session
    """
    from callosum.db.models import ToolCall__SearchDoc
