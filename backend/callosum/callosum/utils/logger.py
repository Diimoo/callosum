...
class CallosumRequestIDFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        from shared_configs.contextvars import CALLOSUM_REQUEST_ID_CONTEXTVAR

        record.request_id = CALLOSUM_REQUEST_ID_CONTEXTVAR.get() or "-"
        return True


class CallosumLoggingAdapter(logging.LoggerAdapter):
    def process(
        ...
        fastapi_request_id = CALLOSUM_REQUEST_ID_CONTEXTVAR.get()
...
def setup_logger(
    ...
    if LOG_FILE_NAME and (is_containerized or DEV_LOGGING_ENABLED):
        ...
        for level in log_levels:
            file_name = (
                f"/var/log/callosum/{LOG_FILE_NAME}_{level}.log"
                if is_containerized
...
def setup_uvicorn_logger(
    ...
    uvicorn_logger.addFilter(CallosumRequestIDFilter())
