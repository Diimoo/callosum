...
The Callosum file store provides a unified interface for storing files and large binary objects in S3-compatible storage systems. It supports AWS S3, MinIO, Azure Blob Storage, Digital Ocean Spaces, and other S3-compatible services.
...
```bash
S3_FILE_STORE_BUCKET_NAME=your-bucket-name  # Defaults to 'callosum-file-store-bucket'
S3_FILE_STORE_PREFIX=callosum-files  # Optional, defaults to 'callosum-files'
