...
from callosum.evals.models import EvalationAck
...
def run_remote(
    ...
    Args:
        base_url: Base URL of the remote server (e.g., "https://test.callosum.app")
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-url",
        type=str,
        default="https://test.callosum.app",
        help="Base URL of the remote server (default: https://test.callosum.app)",
    )
    ...
    if args.remote:
        if not args.api_key:
            print("Using API Key from CALLOSUM_EVAL_API_KEY")
        api_key: str = (
            args.api_key if args.api_key else os.environ.get("CALLOSUM_EVAL_API_KEY", "")
        )
