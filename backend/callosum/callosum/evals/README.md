# Callosum Evaluations

This directory contains the evaluation framework for testing and measuring the performance of Callosum's chat and retrieval systems.

## Overview

The evaluation system uses [Braintrust](https://www.braintrust.dev/) to run automated evaluations against test datasets. It measures the quality of responses generated by Callosum's chat system and can be used to track performance improvements over time.
...
```bash
callosum/backend$ python -m dotenv -f .vscode/.env run -- python callosum/evals/eval_cli.py --remote --api-key <SUPER_CLOUD_USER_API_KEY> --search-permissions-email <email account to reference> --remote --remote-dataset-name Simple
```

You can also run the CLI directly from the command line:

```bash
callosum$ python -m dotenv -f .vscode/.env run -- python backend/callosum/evals/eval_cli.py --local-dataset-path backend/callosum/evals/data/eval.json --search-permissions-email richard@callosum.app
```
make sure your AUTH_TYPE=disabled when running evals locally. Save the env var CALLOSUM_EVAL_API_KEY in your .env file so you don't 
