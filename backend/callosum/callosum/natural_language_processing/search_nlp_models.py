...
from callosum.configs.app_configs import INDEXING_EMBEDDING_MODEL_NUM_THREADS
from callosum.configs.app_configs import LARGE_CHUNK_RATIO
from callosum.configs.model_configs import BATCH_SIZE_ENCODE_CHUNKS
from callosum.configs.model_configs import (
...
from callosum.natural_language_processing.constants import DEFAULT_COHERE_MODEL
from callosum.natural_language_processing.constants import DEFAULT_OPENAI_MODEL
from callosum.natural_language_processing.constants import DEFAULT_VERTEX_MODEL
from callosum.natural_language_processing.constants import DEFAULT_VOYAGE_MODEL
from callosum.natural_language_processing.constants import EmbeddingModelTextType
from callosum.natural_language_processing.exceptions import CohereBillingLimitError
from callosum.natural_language_processing.exceptions import ModelServerRateLimitError
from callosum.natural_language_processing.utils import get_tokenizer
from callosum.natural_language_processing.utils import tokenizer_trim_content
from callosum.utils.logger import setup_logger
from callosum.utils.search_nlp_models_utils import pass_aws_key
...
WARM_UP_STRINGS = [
    ...
    "https://docs.callosum.app/deployment/getting_started/quickstart",
]
...
class CloudEmbedding:
    ...
    async def _embed_cohere(
        ...
        for text_batch in batch_list(texts, _COHERE_MAX_INPUT_LEN):
            # Does not use the same tokenizer as the Callosum API server but it's approximately the same
...
class EmbeddingModel:
    ...
    def _make_model_server_request(
        ...
        def _make_request() -> Response:
            headers = {}
            if tenant_id:
                headers["X-Callosum-Tenant-ID"] = tenant_id

            if request_id:
                headers["X-Callosum-Request-ID"] = request_id
