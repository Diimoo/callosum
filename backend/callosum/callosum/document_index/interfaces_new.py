...
# document as represented in Callosum. What is actually stored in the index is the
...
class IndexRetrievalFilters(BaseModel):
    ...
    Used to filter on permissions and other Callosum-specific metadata rather than
    ...
    def delete(
        ...
        Args:
            document_id: The unique identifier for the document as represented
                in Callosum, not necessarily in the document index.
...
class DocumentIndex(
    ...
    """
    A valid document index that can plug into all Callosum flows must implement all
