...
from callosum.configs.app_configs import EGNYTE_CLIENT_ID
from callosum.configs.app_configs import EGNYTE_CLIENT_SECRET
...
from callosum.connectors.interfaces import OAuthConnector
...
from callosum.utils.retry_wrapper import request_with_retries
...
def _process_egnyte_file(
    ...
    if extension not in CallosumFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
        logger.warning(f"Skipping file '{file_name}' with extension '{extension}'")
        return None

    # Extract text content based on file type
    # TODO @wenxi-callosum: convert to extract_text_and_images
    if extension in CallosumFileExtensions.PLAIN_TEXT_EXTENSIONS:
        encoding = detect_encoding(file_content)
        file_content_raw, file_metadata = read_text_file(
            file_content, encoding=encoding, ignore_callosum_metadata=False
        )
    else:
