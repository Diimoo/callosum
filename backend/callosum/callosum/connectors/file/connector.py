...
from callosum.file_processing.extract_file_text import extract_text_and_images
...
def _process_file(
    ...
    if extension not in CallosumFileExtensions.ALL_ALLOWED_EXTENSIONS:
        logger.warning(
    ...
    callosum_metadata, custom_tags = process_callosum_metadata(metadata)
    file_display_name = callosum_metadata.file_display_name or os.path.basename(file_name)
    time_updated = callosum_metadata.doc_updated_at or datetime.now(timezone.utc)
    primary_owners = callosum_metadata.primary_owners
    secondary_owners = callosum_metadata.secondary_owners
    link = callosum_metadata.link

    # These metadata items are not settable by the user
    source_type = callosum_metadata.source_type or DocumentSource.FILE

    doc_id = f"FILE_CONNECTOR__{file_id}"
    title = metadata.get("title") or file_display_name

    # 1) If the file itself is an image, handle that scenario quickly
    if extension in CallosumFileExtensions.IMAGE_EXTENSIONS:
        # Read the image data
    ...
    # Each file may have file-specific CALLOSUM_METADATA https://docs.callosum.app/admins/connectors/official/file
    ...
    if extraction_result.metadata:
        ...
        callosum_metadata, more_custom_tags = process_callosum_metadata(
        ...
        source_type = callosum_metadata.source_type or source_type
        primary_owners = callosum_metadata.primary_owners or primary_owners
        secondary_owners = callosum_metadata.secondary_owners or secondary_owners
        time_updated = callosum_metadata.doc_updated_at or time_updated
        file_display_name = callosum_metadata.file_display_name or file_display_name
        title = callosum_metadata.title or callosum_metadata.file_display_name or title
        link = callosum_metadata.link or link
