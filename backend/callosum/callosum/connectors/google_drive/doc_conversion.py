...
from callosum.connectors.google_drive.constants import DRIVE_FOLDER_TYPE
from callosum.connectors.google_drive.constants import DRIVE_SHORTCUT_TYPE
from callosum.connectors.google_drive.models import GDriveMimeType
from callosum.connectors.google_drive.models import GoogleDriveFileType
from callosum.connectors.google_drive.section_extraction import get_document_sections
from callosum.connectors.google_drive.section_extraction import HEADING_DELIMITER
...
from callosum.connectors.google_utils.resources import GoogleDocsService
...
from callosum.file_processing.extract_file_text import pptx_to_text
...
from callosum.file_processing.extract_file_text import xlsx_to_text
...
from callosum.file_processing.image_utils import store_image_and_create_section
...
def callosum_document_id_from_drive_file(file: GoogleDriveFileType) -> str:
...
def _download_and_extract_sections_basic(
    ...
    if file_ext not in CallosumFileExtensions.ALL_ALLOWED_EXTENSIONS:
...
def _get_external_access_for_raw_gdrive_file(
    ...
    external_access_fn = cast(
        ...
        fetch_versioned_implementation_with_fallback(
            "callosum.external_permissions.google_drive.doc_sync",
...
def _convert_drive_item_to_document(
    ...
    try:
        ...
        doc_id = callosum_document_id_from_drive_file(file)
        external_access = (
...
def build_slim_document(
    ...
    return SlimDocument(
        id=callosum_document_id_from_drive_file(file),
        external_access=external_access,
    )
