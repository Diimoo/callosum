...
from callosum.connectors.slack.utils import CALLOSUM_SLACK_LOCK_BLOCKING_TIMEOUT
from callosum.connectors.slack.utils import CALLOSUM_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT
from callosum.connectors.slack.utils import CALLOSUM_SLACK_LOCK_TTL
from callosum.utils.logger import setup_logger

logger = setup_logger()


class CallosumSlackWebClient(WebClient):
    """Use in combination with the Callosum Retry Handler.
    ...
    def _perform_urllib_http_request(
        ...
        lock: RedisLock = self._redis.lock(
            self._delay_lock,
            timeout=CALLOSUM_SLACK_LOCK_TTL,
        )

        # try to acquire the lock
        start = time.monotonic()
        while True:
            acquired = lock.acquire(blocking_timeout=CALLOSUM_SLACK_LOCK_BLOCKING_TIMEOUT)
            if acquired:
                break
            ...
            if time.monotonic() - start > CALLOSUM_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT:
                raise RuntimeError(
                    f"CallosumSlackWebClient._perform_urllib_http_request - "
                    f"timed out waiting for lock: {CALLOSUM_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT=}"
                )
        ...
        try:
            ...
            if lock.owned():
                ...
                logger.warning(
                    "CallosumSlackWebClient._perform_urllib_http_request lock not owned on release"
                )

        time.monotonic() - start
        # logger.info(
        #     f"CallosumSlackWebClient._perform_urllib_http_request: Releasing lock: {elapsed=}"
        # )
    ...
    def _perform_urllib_http_request_internal(
        ...
        if delay_ms > 0:
            logger.warning(
                f"CallosumSlackWebClient._perform_urllib_http_request_internal delay: "
                f"{delay_ms=} "
                f"{self.num_requests=}"
            )
