...
class CallosumRedisSlackRetryHandler(RetryHandler):
    """
    ...
    via an override of _perform_urllib_http_request in CallosumSlackWebClient.
        ...
        *,
        ...
        try:
            if retry_after_header_name is None:
                ...
                raise ValueError(
                    "CallosumRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header name is None"
                )

            retry_after_header_value = response.headers.get(retry_after_header_name)
            if not retry_after_header_value:
                raise ValueError(
                    "CallosumRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header value is None"
                )
        ...
        logger.warning(
            f"CallosumRedisSlackRetryHandler.prepare_for_next_attempt setting delay: "
