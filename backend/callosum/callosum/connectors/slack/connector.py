...
from callosum.configs.app_configs import ENABLE_EXPENSIVE_EXPERT_CALLS
from callosum.configs.app_configs import INDEX_BATCH_SIZE
from callosum.configs.app_configs import SLACK_NUM_THREADS
...
from callosum.connectors.slack.access import get_channel_access
from callosum.connectors.slack.models import ChannelType
from callosum.connectors.slack.models import MessageType
from callosum.connectors.slack.models import ThreadType
from callosum.connectors.slack.callosum_retry_handler import CallosumRedisSlackRetryHandler
from callosum.connectors.slack.callosum_slack_web_client import CallosumSlackWebClient
from callosum.connectors.slack.utils import (
    expert_info_from_slack_id,
)
from callosum.connectors.slack.utils import get_message_link
from callosum.connectors.slack.utils import make_paginated_slack_api_call
...
class SlackConnector(
    ...
    def make_slack_web_client(
        ...
        # for concurrent workers. We've extended it with CallosumRedisSlackRetryHandler.
        ...
        callosum_rate_limit_error_retry_handler = CallosumRedisSlackRetryHandler(
            max_retry_count=max_retry_count,
            delay_key=delay_key,
            r=r,
        )
        custom_retry_handlers: list[RetryHandler] = [
            ...
            callosum_rate_limit_error_retry_handler,
        ]

        client = CallosumSlackWebClient(
...
if __name__ == "__main__":
    ...
    provider = CallosumStaticCredentialsProvider(
