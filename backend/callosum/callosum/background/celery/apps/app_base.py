...
from callosum.background.celery.apps.task_formatters import CeleryTaskColoredFormatter
from callosum.background.celery.apps.task_formatters import CeleryTaskPlainFormatter
from callosum.background.celery.celery_utils import celery_is_worker_primary
from callosum.background.celery.celery_utils import make_probe_path
from callosum.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_PREFIX
from callosum.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_TASKSET_KEY
...
from callosum.db.engine.sql_engine import get_sqlalchemy_engine
from callosum.document_index.vespa.shared_utils.utils import wait_for_vespa_with_timeout
...
from callosum.utils.logger import ColoredFormatter
from callosum.utils.logger import LoggerContextVars
from callosum.utils.logger import PlainFormatter
...
def on_task_postrun(
    ...
    **kwds: Any,
    ...
    if task.name.startswith(CALLOSUM_CLOUD_CELERY_TASK_PREFIX):
...
def on_secondary_worker_init(sender: Any, **kwargs: Any) -> None:
    ...
    while True:
        if r.exists(CallosumRedisLocks.PRIMARY_WORKER):
            break
...
class TenantContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if tenant_id:
            # Match the 8 character tenant abbreviation used in CallosumLoggingAdapter
