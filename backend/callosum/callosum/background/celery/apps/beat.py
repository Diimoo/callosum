...
from callosum.configs.constants import POSTGRES_CELERY_BEAT_APP_NAME
...
celery_app.config_from_object("callosum.background.celery.configs.beat")
    ...
    def _generate_schedule(
        ...
        if MULTI_TENANT:
            ...
            get_cloud_tasks_to_schedule = fetch_versioned_implementation(
                "callosum.background.celery.tasks.beat_schedule",
        ...
        get_tasks_to_schedule = fetch_versioned_implementation(
            "callosum.background.celery.tasks.beat_schedule", "get_tasks_to_schedule"
        )
