...
from callosum.configs.constants import CALLOSUM_CELERY_BEAT_HEARTBEAT_KEY
from callosum.configs.constants import CallosumCeleryTask
from callosum.db.document import delete_document_by_connector_credential_pair__no_commit
from callosum.db.document import delete_documents_complete__no_commit
from callosum.db.document import fetch_chunk_count_for_document
from callosum.db.document import get_document
from callosum.db.document import get_document_connector_count
from callosum.db.document import mark_document_as_modified
...
from callosum.db.relationships import delete_document_references_from_kg
...
class CallosumCeleryTaskCompletionStatus(str, Enum):
...
@shared_task(
    name=CallosumCeleryTask.DOCUMENT_BY_CC_PAIR_CLEANUP_TASK,
...
def document_by_cc_pair_cleanup_task(
    ...
    if completion_status != CallosumCeleryTaskCompletionStatus.SUCCEEDED:
        return False
...
@shared_task(name=CallosumCeleryTask.CELERY_BEAT_HEARTBEAT, ignore_result=True, bind=True)
def celery_beat_heartbeat(self: Task, *, tenant_id: str) -> None:
    ...
    r.set(CALLOSUM_CELERY_BEAT_HEARTBEAT_KEY, 1, ex=600)
