...
from callosum.configs.constants import CELERY_USER_FILE_DOCID_MIGRATION_LOCK_TIMEOUT
from callosum.configs.constants import CELERY_USER_FILE_PROCESSING_LOCK_TIMEOUT
from callosum.configs.constants import CELERY_USER_FILE_PROJECT_SYNC_LOCK_TIMEOUT
...
from callosum.db.models import FileRecord
from callosum.db.models import SearchDoc
from callosum.db.models import UserFile
from callosum.db.search_settings import get_active_search_settings
from callosum.db.search_settings import get_active_search_settings_list
...
from callosum.file_store.file_store import S3BackedFileStore
from callosum.file_store.utils import user_file_id_to_plaintext_file_name
from callosum.httpx.httpx_pool import HttpxPool
from callosum.indexing.adapters.user_file_indexing_adapter import UserFileIndexingAdapter
...
def _user_file_lock_key(user_file_id: str | UUID) -> str:
    return f"{CallosumRedisLocks.USER_FILE_PROCESSING_LOCK_PREFIX}:{user_file_id}"


def _user_file_project_sync_lock_key(user_file_id: str | UUID) -> str:
    return f"{CallosumRedisLocks.USER_FILE_PROJECT_SYNC_LOCK_PREFIX}:{user_file_id}"


def _user_file_delete_lock_key(user_file_id: str | UUID) -> str:
    return f"{CallosumRedisLocks.USER_FILE_DELETE_LOCK_PREFIX}:{user_file_id}"
...
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_USER_FILE_PROCESSING,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_user_file_processing(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        CallosumRedisLocks.USER_FILE_PROCESSING_BEAT_LOCK,
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                self.app.send_task(
                    CallosumCeleryTask.PROCESS_SINGLE_USER_FILE,
                    kwargs={"user_file_id": str(user_file_id), "tenant_id": tenant_id},
                    queue=CallosumCeleryQueues.USER_FILE_PROCESSING,
...
@shared_task(
    name=CallosumCeleryTask.PROCESS_SINGLE_USER_FILE,
    bind=True,
    ignore_result=True,
)
...
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_USER_FILE_DELETE,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_user_file_delete(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        CallosumRedisLocks.USER_FILE_DELETE_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    if not lock.acquire(blocking=False):
        return None
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                self.app.send_task(
                    CallosumCeleryTask.DELETE_SINGLE_USER_FILE,
                    kwargs={"user_file_id": str(user_file_id), "tenant_id": tenant_id},
                    queue=CallosumCeleryQueues.USER_FILE_DELETE,
                    priority=CallosumCeleryPriority.HIGH,
                )
                enqueued += 1
    except Exception as e:
...
@shared_task(
    name=CallosumCeleryTask.DELETE_SINGLE_USER_FILE,
    bind=True,
    ignore_result=True,
)
...
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_USER_FILE_PROJECT_SYNC,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_user_file_project_sync(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        CallosumRedisLocks.USER_FILE_PROJECT_SYNC_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                self.app.send_task(
                    CallosumCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
                    kwargs={"user_file_id": str(user_file_id), "tenant_id": tenant_id},
                    queue=CallosumCeleryQueues.USER_FILE_PROJECT_SYNC,
                    priority=CallosumCeleryPriority.HIGH,
                )
                enqueued += 1
    finally:
...
@shared_task(
    name=CallosumCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
    bind=True,
    ignore_result=True,
)
...
@shared_task(
    name=CallosumCeleryTask.USER_FILE_DOCID_MIGRATION,
    ignore_result=True,
    bind=True,
)
def user_file_docid_migration_task(self: Task, *, tenant_id: str) -> bool:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        CallosumRedisLocks.USER_FILE_DOCID_MIGRATION_LOCK,
