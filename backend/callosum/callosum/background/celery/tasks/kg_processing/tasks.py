...
from callosum.kg.clustering.clustering import kg_clustering
from callosum.kg.extractions.extraction_processing import kg_extraction
from callosum.kg.resets.reset_source import reset_source_kg_index
...
@shared_task(
    name=CallosumCeleryTask.CHECK_KG_PROCESSING,
    soft_time_limit=300,
    bind=True,
)
def check_for_kg_processing(self: Task, *, tenant_id: str) -> None:
    ...
    try:
        ...
        self.app.send_task(
            CallosumCeleryTask.KG_PROCESSING,
            kwargs={
                "tenant_id": tenant_id,
            },
            queue=CallosumCeleryQueues.KG_PROCESSING,
...
@shared_task(
    name=CallosumCeleryTask.CHECK_KG_PROCESSING_CLUSTERING_ONLY,
    soft_time_limit=300,
    bind=True,
)
def check_for_kg_processing_clustering_only(self: Task, *, tenant_id: str) -> None:
    ...
    try:
        ...
        self.app.send_task(
            CallosumCeleryTask.KG_CLUSTERING_ONLY,
            kwargs={
                "tenant_id": tenant_id,
            },
            queue=CallosumCeleryQueues.KG_PROCESSING,
...
@shared_task(
    name=CallosumCeleryTask.KG_PROCESSING,
    bind=True,
)
def kg_processing(self: Task, *, tenant_id: str) -> None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        CallosumRedisLocks.KG_PROCESSING_LOCK,
...
@shared_task(
    name=CallosumCeleryTask.KG_CLUSTERING_ONLY,
    bind=True,
)
...
@shared_task(
    name=CallosumCeleryTask.KG_RESET_SOURCE_INDEX,
    bind=True,
)
