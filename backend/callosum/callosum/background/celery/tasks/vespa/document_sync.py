...
from callosum.db.document import construct_document_id_select_by_needs_sync
...
def set_document_sync_fence(r: Redis, payload: int | None) -> None:
    ...
    if payload is None:
        r.srem(CallosumRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
        r.delete(DOCUMENT_SYNC_FENCE_KEY)
        return

    r.set(DOCUMENT_SYNC_FENCE_KEY, payload)
    r.sadd(CallosumRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
