...
from callosum.background.celery.celery_utils import extract_ids_from_runnable_connector
from callosum.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from callosum.background.celery.tasks.docprocessing.utils import IndexingCallbackBase
from callosum.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
...
from callosum.configs.constants import CELERY_PRUNING_LOCK_TIMEOUT
...
from callosum.connectors.factory import instantiate_connector
...
from callosum.redis.redis_connector_prune import RedisConnectorPrunePayload
...
from callosum.utils.logger import pruning_ctx
...
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        CallosumRedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(CallosumRedisSignals.BLOCK_PRUNING):
            ...
            r.set(CallosumRedisSignals.BLOCK_PRUNING, 1, ex=_get_pruning_block_expiration())

        # we want to run this less frequently than the overall task
        lock_beat.reacquire()
        if not r.exists(CallosumRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(
                CallosumRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES,
                1,
...
def try_creating_prune_generator_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            CallosumCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=CallosumCeleryQueues.CONNECTOR_PRUNING,
...
@shared_task(
    name=CallosumCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        CallosumRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.cc_pair_id}",
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(CallosumCeleryQueues.CONNECTOR_DELETION, r_celery)
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        CallosumCeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:
