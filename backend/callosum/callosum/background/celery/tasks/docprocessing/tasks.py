...
from callosum.background.celery.celery_redis import celery_find_task
from callosum.background.celery.celery_redis import celery_get_unacked_task_ids
from callosum.background.celery.celery_utils import httpx_init_vespa_pool
from callosum.background.celery.memory_monitoring import emit_process_memory
from callosum.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from callosum.background.celery.tasks.docfetching.task_creation_utils import (
    try_creating_docfetching_task,
)
from callosum.background.celery.tasks.docprocessing.heartbeat import start_heartbeat
from callosum.background.celery.tasks.docprocessing.heartbeat import stop_heartbeat
from callosum.background.celery.tasks.docprocessing.utils import IndexingCallback
from callosum.background.celery.tasks.docprocessing.utils import is_in_repeated_error_state
from callosum.background.celery.tasks.docprocessing.utils import should_index
from callosum.background.celery.tasks.models import DocProcessingContext
from callosum.background.indexing.checkpointing_utils import cleanup_checkpoint
from callosum.background.indexing.checkpointing_utils import (
    get_index_attempts_with_old_checkpoints,
)
from callosum.background.indexing.index_attempt_utils import cleanup_index_attempts
from callosum.background.indexing.index_attempt_utils import get_old_index_attempts
from callosum.configs.app_configs import MANAGED_VESPA
from callosum.configs.app_configs import VESPA_CLOUD_CERT_PATH
from callosum.configs.app_configs import VESPA_CLOUD_KEY_PATH
from callosum.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from callosum.configs.constants import CELERY_INDEXING_LOCK_TIMEOUT
...
from callosum.configs.constants import CallosumRedisConstants
from callosum.configs.constants import CallosumRedisLocks
from callosum.configs.constants import CallosumRedisSignals
from callosum.connectors.models import ConnectorFailure
from callosum.connectors.models import Document
from callosum.connectors.models import IndexAttemptMetadata
from callosum.db.connector import mark_ccpair_with_indexing_trigger
...
from callosum.db.connector_credential_pair import get_connector_credential_pair_from_id
from callosum.db.connector_credential_pair import set_cc_pair_repeated_error_state
from callosum.db.engine.sql_engine import get_session_with_current_tenant
from callosum.db.engine.time_utils import get_db_current_time
from callosum.db.enums import ConnectorCredentialPairStatus
from callosum.db.enums import IndexingMode
from callosum.db.enums import IndexingStatus
from callosum.db.enums import SwitchoverType
from callosum.db.index_attempt import create_index_attempt_error
from callosum.db.index_attempt import get_index_attempt
from callosum.db.index_attempt import get_index_attempt_errors_for_cc_pair
from callosum.db.index_attempt import IndexAttemptError
from callosum.db.index_attempt import mark_attempt_canceled
from callosum.db.index_attempt import mark_attempt_failed
from callosum.db.index_attempt import mark_attempt_partially_succeeded
from callosum.db.index_attempt import mark_attempt_succeeded
from callosum.db.indexing_coordination import CoordinationStatus
from callosum.db.indexing_coordination import INDEXING_PROGRESS_TIMEOUT_HOURS
from callosum.db.indexing_coordination import IndexingCoordination
...
from callosum.db.swap_index import check_and_perform_index_swap
from callosum.document_index.factory import get_default_document_index
from callosum.file_store.document_batch_storage import DocumentBatchStorage
from callosum.file_store.document_batch_storage import get_document_batch_storage
from callosum.httpx.httpx_pool import HttpxPool
from callosum.indexing.adapters.document_indexing_adapter import (
    DocumentIndexingBatchAdapter,
)
from callosum.indexing.embedder import DefaultIndexingEmbedder
from callosum.indexing.indexing_pipeline import run_indexing_pipeline
from callosum.natural_language_processing.search_nlp_models import EmbeddingModel
from callosum.natural_language_processing.search_nlp_models import (
    InformationContentClassificationModel,
)
from callosum.natural_language_processing.search_nlp_models import warm_up_bi_encoder
from callosum.redis.redis_connector import RedisConnector
from callosum.redis.redis_pool import get_redis_client
from callosum.redis.redis_pool import get_redis_replica_client
from callosum.redis.redis_pool import redis_lock_dump
from callosum.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
from callosum.redis.redis_utils import is_fence
from callosum.server.runtime.callosum_runtime import CallosumRuntime
from callosum.utils.logger import setup_logger
from callosum.utils.middleware import make_randomized_callosum_request_id
from callosum.utils.telemetry import mt_cloud_telemetry
...
def _get_fence_validation_block_expiration() -> int:
    ...
    try:
        beat_multiplier = CallosumRuntime.get_beat_multiplier()
    except Exception:
        ...
        if attempt and timed_out:
            if attempt.status == IndexingStatus.IN_PROGRESS:
                ...
                task_exists = celery_find_task(
                    ...
                    CallosumCeleryQueues.CONNECTOR_DOC_FETCHING,
                    redis_celery,
                )
                unacked_task_ids = celery_get_unacked_task_ids(
                    CallosumCeleryQueues.CONNECTOR_DOC_FETCHING, redis_celery
                )
...
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    the docfetching task (CallosumCeleryTask.CONNECTOR_DOC_FETCHING_TASK).
    ...
    lock_beat: RedisLock = redis_client.lock(
        CallosumRedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(CallosumRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    CallosumRedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning(f"Adding {key_bytes} to the lookup table.")
                    redis_client.sadd(CallosumRedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                CallosumRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=CallosumRuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(CallosumRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(
                CallosumRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES,
                1,
...
# primary
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        CallosumRedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.app.send_task(
                    CallosumCeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=CallosumCeleryQueues.CHECKPOINT_CLEANUP,
...
# light worker
@shared_task(
    name=CallosumCeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
...
# primary
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_index_attempt_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        CallosumRedisLocks.CHECK_INDEX_ATTEMPT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for i in range(0, len(old_attempts), batch_size):
                ...
                self.app.send_task(
                    CallosumCeleryTask.CLEANUP_INDEX_ATTEMPT,
                    kwargs={
                        "index_attempt_ids": [attempt.id for attempt in batch],
                        "tenant_id": tenant_id,
                    },
                    queue=CallosumCeleryQueues.INDEX_ATTEMPT_CLEANUP,
                    priority=CallosumCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(
    name=CallosumCeleryTask.CLEANUP_INDEX_ATTEMPT,
    bind=True,
)
...
@shared_task(
    name=CallosumCeleryTask.DOCPROCESSING_TASK,
    bind=True,
)
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            index_attempt_metadata = IndexAttemptMetadata(
                ...
                request_id=make_randomized_callosum_request_id("DIP"),
