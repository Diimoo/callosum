...
from callosum.db.engine.sql_engine import get_session_with_shared_schema
...
from callosum.db.models import SyncRecord
...
from callosum.utils.logger import is_running_in_container
...
def _collect_queue_metrics(redis_celery: Redis) -> list[Metric]:
    ...
    queue_mappings = {
        ...
        "permissions_sync_queue_length": CallosumCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        "external_group_sync_queue_length": CallosumCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
        "permissions_upsert_queue_length": CallosumCeleryQueues.DOC_PERMISSIONS_UPSERT,
    }
...
@shared_task(
    name=CallosumCeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ignore_result=True,
...
def monitor_background_processes(self: Task, *, tenant_id: str) -> None:
    ...
    lock_monitoring: RedisLock = r.lock(
        CallosumRedisLocks.MONITOR_BACKGROUND_PROCESSES_LOCK,
...
@shared_task(
    name=CallosumCeleryTask.CLOUD_MONITOR_ALEMBIC,
)
def cloud_check_alembic() -> bool | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        CallosumRedisLocks.CLOUD_CHECK_ALEMBIC_BEAT_LOCK,
...
@shared_task(
    name=CallosumCeleryTask.CLOUD_MONITOR_CELERY_QUEUES, ignore_result=True, bind=True
)
...
@shared_task(name=CallosumCeleryTask.MONITOR_CELERY_QUEUES, ignore_result=True, bind=True)
...
def monitor_celery_queues_helper(
    ...
    n_docfetching = celery_get_queue_length(
        CallosumCeleryQueues.CONNECTOR_DOC_FETCHING, r_celery
    )
    n_docprocessing = celery_get_queue_length(CallosumCeleryQueues.DOCPROCESSING, r_celery)
    n_user_files_indexing = celery_get_queue_length(
        CallosumCeleryQueues.USER_FILES_INDEXING, r_celery
    )
    n_user_file_processing = celery_get_queue_length(
        CallosumCeleryQueues.USER_FILE_PROCESSING, r_celery
    )
    n_user_file_project_sync = celery_get_queue_length(
        CallosumCeleryQueues.USER_FILE_PROJECT_SYNC, r_celery
    )
    n_user_file_delete = celery_get_queue_length(
        CallosumCeleryQueues.USER_FILE_DELETE, r_celery
    )
    n_sync = celery_get_queue_length(CallosumCeleryQueues.VESPA_METADATA_SYNC, r_celery)
    n_deletion = celery_get_queue_length(CallosumCeleryQueues.CONNECTOR_DELETION, r_celery)
    n_pruning = celery_get_queue_length(CallosumCeleryQueues.CONNECTOR_PRUNING, r_celery)
    ...
    n_docprocessing_prefetched = celery_get_unacked_task_ids(
        CallosumCeleryQueues.DOCPROCESSING, r_celery
    )
...
@shared_task(
    name=CallosumCeleryTask.MONITOR_PROCESS_MEMORY,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=CallosumCeleryQueues.MONITORING,
...
@shared_task(
    name=CallosumCeleryTask.CLOUD_MONITOR_CELERY_PIDBOX, ignore_result=True, bind=True
)
