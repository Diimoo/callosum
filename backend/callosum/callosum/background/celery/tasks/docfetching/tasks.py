...
from callosum.background.celery.tasks.docprocessing.tasks import ConnectorIndexingLogBuilder
...
from callosum.background.celery.tasks.models import IndexingWatchdogTerminalStatus
from callosum.background.celery.tasks.models import SimpleJobResult
from callosum.background.indexing.job_client import SimpleJob
from callosum.background.indexing.job_client import SimpleJobClient
from callosum.background.indexing.job_client import SimpleJobException
from callosum.background.indexing.run_docfetching import run_docfetching_entrypoint
from callosum.configs.constants import CELERY_INDEXING_WATCHDOG_CONNECTOR_TIMEOUT
...
@shared_task(
    name=CallosumCeleryTask.CONNECTOR_DOC_FETCHING_TASK,
    bind=True,
    acks_late=False,
    track_started=True,
)
def docfetching_proxy_task(
    ...
    1)  determines parameters of the indexing attempt (which connector indexing function to run,
        ...
        connectors are responsible for reading data from an outside source and converting it to Callosum documents.
        At the moment these two steps (reading external data and converting to an Callosum document)
