...
from callosum.configs.app_configs import DB_YIELD_PER_DEFAULT
...
from callosum.db.document import construct_document_id_select_for_connector_credential_pair
...
class RedisConnectorDelete:
    ...
    def set_fence(self, payload: RedisConnectorDeletePayload | None) -> None:
        if not payload:
            self.redis.srem(CallosumRedisConstants.ACTIVE_FENCES, self.fence_key)
            self.redis.delete(self.fence_key)
            return

        self.redis.set(self.fence_key, payload.model_dump_json(), ex=self.FENCE_TTL)
        self.redis.sadd(CallosumRedisConstants.ACTIVE_FENCES, self.fence_key)
    ...
    def generate_tasks(
        ...
        for doc_id in db_session.scalars(stmt).yield_per(DB_YIELD_PER_DEFAULT):
            ...
            celery_app.send_task(
                CallosumCeleryTask.DOCUMENT_BY_CC_PAIR_CLEANUP_TASK,
                kwargs=dict(
                    document_id=doc_id,
                ...
                queue=CallosumCeleryQueues.CONNECTOR_DELETION,
