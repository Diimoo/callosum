...
from callosum.file_processing.file_types import PRESENTATION_MIME_TYPE
from callosum.file_processing.file_types import WORD_PROCESSING_MIME_TYPE
from callosum.file_processing.html_utils import parse_html_page_basic
from callosum.file_processing.unstructured import get_unstructured_api_key
from callosum.file_processing.unstructured import unstructured_to_text
...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        for file_info in zip_file.infolist():
            ...
            ) or file_info.filename == CALLOSUM_METADATA_FILENAME:
                continue
...
def _extract_callosum_metadata(line: str) -> dict | None:
    """
    Example: first line has:
        <!-- CALLOSUM_METADATA={"title": "..."} -->
      or
        #CALLOSUM_METADATA={"title":"..."}
    """
    html_comment_pattern = r"<!--\s*CALLOSUM_METADATA=\{(.*?)\}\s*-->"
    hashtag_pattern = r"#CALLOSUM_METADATA=\{(.*?)\}"
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_callosum_metadata: bool = True,
) -> tuple[str, dict]:
    """
    For plain text files. Optionally extracts Callosum metadata from the first line.
    """
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_callosum_metadata:
            potential_meta = _extract_callosum_metadata(line)
...
def extract_file_text(
    ...
    try:
        ...
        if extension in CallosumFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
...
def _extract_text_and_images(
    ...
    if content_type in CallosumMimeTypes.TEXT_MIME_TYPES:
    ...
    try:
        ...
        if extension in CallosumFileExtensions.PLAIN_TEXT_EXTENSIONS:
            return extract_result_from_text_file(file)
