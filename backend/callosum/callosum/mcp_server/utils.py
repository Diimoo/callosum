"""Utility helpers for the Callosum MCP server."""
...
def require_access_token() -> AccessToken:
    ...
    if not access_token:
        raise ValueError(
            "MCP Server requires an Callosum access token to authenticate your request"
        )
    return access_token
...
def get_api_server_url() -> str:
    ...
    override = os.getenv("API_SERVER_BASE_URL") or os.getenv("CALLOSUM_URL")
    if override:
...
async def get_indexed_sources(
    ...
    try:
        ...
        logger.error(
            "Callosum MCP Server: Failed to fetch indexed sources",
            exc_info=True,
        )
        raise
        ...
        logger.error(
            "Callosum MCP Server: Unexpected error fetching indexed sources",
            exc_info=True,
        )
