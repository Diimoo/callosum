...
from callosum.configs.app_configs import MCP_SERVER_CORS_ORIGINS
from callosum.mcp_server.auth import CallosumTokenVerifier
from callosum.mcp_server.utils import shutdown_http_client
from callosum.utils.logger import setup_logger

logger = setup_logger()

logger.info("Creating Callosum MCP Server...")

mcp_server = FastMCP(
    name="Callosum MCP Server",
    version="1.0.0",
    auth=CallosumTokenVerifier(),
)
...
from callosum.mcp_server.tools import search  # noqa: E402, F401
from callosum.mcp_server.resources import indexed_sources  # noqa: E402, F401
...
def create_mcp_fastapi_app() -> FastAPI:
    ...
    # Authentication is handled by FastMCP's CallosumTokenVerifier (see auth.py)
