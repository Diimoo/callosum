...
from callosum.mcp_server.api import mcp_server
from callosum.mcp_server.utils import get_api_server_url
from callosum.mcp_server.utils import get_http_client
from callosum.mcp_server.utils import get_indexed_sources
from callosum.mcp_server.utils import require_access_token
...
async def search_indexed_documents(
    ...
    """
    Search the user's knowledge base indexed in Callosum.
    ...
    """
    logger.info(
        f"Callosum MCP Server: document search: query='{query}', sources={source_types}, limit={limit}"
    )
    ...
    if time_cutoff:
        try:
            ...
            logger.warning(
                f"Callosum MCP Server: Invalid time_cutoff format '{time_cutoff}': {e}. "
    ...
    try:
        ...
        logger.error(
            "Callosum MCP Server: Error checking indexed sources: %s",
            e,
            exc_info=True,
        )
        return {
    ...
    if not sources:
        logger.info("Callosum MCP Server: No indexed sources available for tenant")
        return {
            "documents": [],
            "total_results": 0,
            "query": query,
            "message": (
                ...
                "through Callosum before calling callosum_search_documents."
            ),
        }
    ...
    if source_types is not None:
        ...
        for src in source_types:
            try:
                ...
                logger.warning(
                    f"Callosum MCP Server: Invalid source type '{src}' - will be ignored by server"
                )
    ...
    try:
        ...
        logger.info(
            f"Callosum MCP Server: Internal search returned {len(documents)} results"
        )
        return {
            "documents": documents,
        ...
        logger.error(f"Callosum MCP Server: Document search error: {e}", exc_info=True)
        return {
...
async def search_web(
    ...
    ```
    """
    logger.info(f"Callosum MCP Server: Web search: query='{query}', limit={limit}")
    ...
    try:
        ...
        logger.error(f"Callosum MCP Server: Web search error: {e}", exc_info=True)
        return {
...
async def open_urls(
    ...
    ```
    """
    logger.info(f"Callosum MCP Server: Open URL: fetching {len(urls)} URLs")
    ...
    try:
        ...
        logger.error(f"Callosum MCP Server: URL fetch error: {e}", exc_info=True)
        return {
