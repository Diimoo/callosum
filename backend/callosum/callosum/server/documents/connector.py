...
from callosum.auth.users import current_chat_accessible_user
from callosum.auth.users import current_curator_or_admin_user
from callosum.auth.users import current_user
from callosum.background.celery.tasks.pruning.tasks import (
    try_creating_prune_generator_task,
)
from callosum.background.celery.versioned_apps.client import app as client_app
from callosum.configs.app_configs import DISABLE_AUTH
from callosum.configs.app_configs import ENABLED_CONNECTOR_TYPES
from callosum.configs.app_configs import MOCK_CONNECTOR_FILE_PATH
...
from callosum.configs.constants import CALLOSUM_METADATA_FILENAME
...
from callosum.connectors.factory import validate_ccpair_for_user
from callosum.connectors.google_utils.google_auth import (
    get_google_oauth_creds,
)
from callosum.connectors.google_utils.google_kv import (
...
from callosum.connectors.google_utils.google_kv import get_auth_url
from callosum.connectors.google_utils.google_kv import (
    get_google_app_cred,
)
from callosum.connectors.google_utils.google_kv import (
...
from callosum.connectors.google_utils.google_kv import verify_csrf
from callosum.connectors.google_utils.shared_constants import DB_CREDENTIALS_DICT_TOKEN_KEY
from callosum.connectors.google_utils.shared_constants import (
    GoogleOAuthAuthenticationMethod,
)
from callosum.db.connector import create_connector
from callosum.db.connector import delete_connector
from callosum.db.connector import fetch_connector_by_id
from callosum.db.connector import fetch_connectors
from callosum.db.connector import fetch_unique_document_sources
from callosum.db.connector import get_connector_credential_ids
from callosum.db.connector import mark_ccpair_with_indexing_trigger
from callosum.db.connector import update_connector
from callosum.db.connector_credential_pair import add_credential_to_connector
from callosum.db.connector_credential_pair import (
    fetch_connector_credential_pair_for_connector,
)
from callosum.db.connector_credential_pair import get_cc_pair_groups_for_ids
from callosum.db.connector_credential_pair import get_connector_credential_pair
from callosum.db.connector_credential_pair import get_connector_credential_pairs_for_user
from callosum.db.connector_credential_pair import (
    get_connector_credential_pairs_for_user_parallel,
)
from callosum.db.credentials import cleanup_gmail_credentials
from callosum.db.credentials import cleanup_google_drive_credentials
from callosum.db.credentials import create_credential
from callosum.db.credentials import delete_service_account_credentials
from callosum.db.credentials import fetch_credential_by_id_for_user
from callosum.db.deletion_attempt import check_deletion_attempt_is_allowed
from callosum.db.document import get_document_counts_for_all_cc_pairs
...
from callosum.db.federated import fetch_all_federated_connectors_parallel
from callosum.db.index_attempt import get_index_attempts_for_cc_pair
from callosum.db.index_attempt import get_latest_index_attempts_by_status
from callosum.db.index_attempt import get_latest_index_attempts_parallel
...
from callosum.db.models import IndexingStatus
from callosum.db.models import User
from callosum.db.models import UserRole
from callosum.file_processing.file_types import PLAIN_TEXT_MIME_TYPE
from callosum.file_processing.file_types import WORD_PROCESSING_MIME_TYPE
from callosum.file_store.file_store import get_default_file_store
from callosum.key_value_store.interface import KvKeyNotFoundError
from callosum.redis.redis_pool import get_redis_client
from callosum.server.documents.models import AuthStatus
from callosum.server.documents.models import AuthUrl
...
from callosum.server.documents.models import ConnectorFileInfo
from callosum.server.documents.models import ConnectorFilesResponse
from callosum.server.documents.models import ConnectorIndexingStatusLite
from callosum.server.documents.models import ConnectorIndexingStatusLiteResponse
from callosum.server.documents.models import ConnectorSnapshot
from callosum.server.documents.models import ConnectorStatus
from callosum.server.documents.models import ConnectorUpdateRequest
from callosum.server.documents.models import CredentialBase
from callosum.server.documents.models import CredentialSnapshot
from callosum.server.documents.models import DocsCountOperator
from callosum.server.documents.models import FailedConnectorIndexingStatus
from callosum.server.documents.models import FileUploadResponse
from callosum.server.documents.models import GDriveCallback
from callosum.server.documents.models import GmailCallback
from callosum.server.documents.models import GoogleAppCredentials
from callosum.server.documents.models import GoogleServiceAccountCredentialRequest
from callosum.server.documents.models import GoogleServiceAccountKey
from callosum.server.documents.models import IndexedSourcesResponse
from callosum.server.documents.models import IndexingStatusRequest
from callosum.server.documents.models import ObjectCreationIdResponse
from callosum.server.documents.models import RunConnectorRequest
from callosum.server.documents.models import SourceSummary
from callosum.server.federated.models import FederatedConnectorStatus
from callosum.server.models import StatusResponse
from callosum.utils.logger import setup_logger
from callosum.utils.telemetry import mt_cloud_telemetry
from callosum.utils.threadpool_concurrency import CallableProtocol
from callosum.utils.threadpool_concurrency import run_functions_tuples_in_parallel
from callosum.utils.variable_functionality import fetch_ee_implementation_or_noop
...
def extract_zip_metadata(zf: zipfile.ZipFile) -> dict[str, Any]:
    zip_metadata = {}
    try:
        metadata_file_info = zf.getinfo(CALLOSUM_METADATA_FILENAME)
        with zf.open(metadata_file_info, "r") as metadata_file:
            try:
                ...
                logger.warning(f"Unable to load {CALLOSUM_METADATA_FILENAME}: {e}")
                # should fail loudly here to let users know that their metadata
                # file is not valid JSON
                raise HTTPException(
                    status_code=400,
                    detail=f"Unable to load {CALLOSUM_METADATA_FILENAME}: {e}",
                )
    except KeyError:
        logger.info(f"No {CALLOSUM_METADATA_FILENAME} file")
    return zip_metadata
    ...
    try:
        ...
        for file in files:
            ...
            if file.content_type == WORD_PROCESSING_MIME_TYPE:
                ...
                from callosum.file_processing.extract_file_text import read_docx_file
    ...
    try:
        ...
        if new_file_paths:
            ...
            client_app.send_task(
                CallosumCeleryTask.CHECK_FOR_INDEXING,
...
def create_connector_from_model(
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "callosum.db.user_group", "validate_object_creation_for_user", None
        )(
            db_session=db_session,
            user=user,
...
def trigger_indexing_for_cc_pair(
    ...
    priority = CallosumCeleryPriority.HIGHEST if is_user_file else CallosumCeleryPriority.HIGH
