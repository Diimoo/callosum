...
PUBLIC_ENDPOINT_SPECS = [
    ...
    ("/auth/type", {"GET"}),
    # just gets the version of Callosum (e.g. 0.3.11)
    ("/version", {"GET"}),
    # Gets stable and beta versions for Callosum docker images
    ("/versions", {"GET"}),
...
def check_router_auth(
    ...
    control_plane_dep = fetch_ee_implementation_or_noop(
        "callosum.server.tenants.access", "control_plane_dep"
    )
    current_cloud_superuser = fetch_ee_implementation_or_noop(
        "callosum.auth.users", "current_cloud_superuser"
    )
