...
from callosum.auth.email_utils import send_user_email_invite
from callosum.auth.invited_users import get_invited_users
from callosum.auth.invited_users import remove_user_from_invited_users
from callosum.auth.invited_users import write_invited_users
from callosum.auth.noauth_user import fetch_no_auth_user
from callosum.auth.noauth_user import set_no_auth_user_personalization
from callosum.auth.noauth_user import set_no_auth_user_preferences
from callosum.auth.schemas import UserRole
from callosum.auth.users import anonymous_user_enabled
...
from callosum.auth.users import optional_user
from callosum.configs.app_configs import AUTH_BACKEND
from callosum.configs.app_configs import AUTH_TYPE
from callosum.configs.app_configs import AuthBackend
from callosum.configs.app_configs import DEV_MODE
from callosum.configs.app_configs import ENABLE_EMAIL_INVITES
from callosum.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from callosum.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
from callosum.configs.app_configs import VALID_EMAIL_DOMAINS
from callosum.configs.constants import AuthType
from callosum.configs.constants import FASTAPI_USERS_AUTH_COOKIE_NAME
from callosum.db.api_key import is_api_key_email_address
from callosum.db.auth import get_live_users_count
from callosum.db.engine.sql_engine import get_session
from callosum.db.enums import UserFileStatus
from callosum.db.models import User
from callosum.db.models import UserFile
from callosum.db.user_preferences import activate_user
from callosum.db.user_preferences import deactivate_user
from callosum.db.user_preferences import get_all_user_assistant_specific_configs
from callosum.db.user_preferences import get_latest_access_token_for_user
from callosum.db.user_preferences import update_assistant_preferences
from callosum.db.user_preferences import update_user_assistant_visibility
from callosum.db.user_preferences import update_user_auto_scroll
from callosum.db.user_preferences import update_user_default_model
from callosum.db.user_preferences import update_user_personalization
from callosum.db.user_preferences import update_user_pinned_assistants
from callosum.db.user_preferences import update_user_role
from callosum.db.user_preferences import update_user_shortcut_enabled
from callosum.db.user_preferences import update_user_temperature_override_enabled
from callosum.db.user_preferences import update_user_theme_preference
from callosum.db.users import delete_user_from_db
from callosum.db.users import get_all_users
from callosum.db.users import get_page_of_filtered_users
from callosum.db.users import get_total_filtered_users_count
from callosum.db.users import get_user_by_email
from callosum.db.users import validate_user_role_update
from callosum.key_value_store.factory import get_kv_store
from callosum.redis.redis_pool import get_raw_redis_client
...
from callosum.server.manage.models import AllUsersResponse
from callosum.server.manage.models import AutoScrollRequest
from callosum.server.manage.models import PersonalizationUpdateRequest
from callosum.server.manage.models import TenantInfo
from callosum.server.manage.models import TenantSnapshot
from callosum.server.manage.models import ThemePreferenceRequest
from callosum.server.manage.models import UserByEmail
from callosum.server.manage.models import UserInfo
from callosum.server.manage.models import UserPreferences
from callosum.server.manage.models import UserRoleResponse
from callosum.server.manage.models import UserRoleUpdateRequest
from callosum.server.manage.models import UserSpecificAssistantPreference
from callosum.server.manage.models import UserSpecificAssistantPreferences
from callosum.server.models import FullUserSnapshot
from callosum.server.models import InvitedUserSnapshot
from callosum.server.models import MinimalUserSnapshot
...
async def test_upsert_user(
    ...
    user = await fetch_ee_implementation_or_noop(
        "callosum.server.saml", "upsert_saml_user", None
    )(email=request.email)
...
def bulk_invite_users(
    ...
    if MULTI_TENANT:
        try:
            fetch_ee_implementation_or_noop(
                "callosum.server.tenants.provisioning", "add_users_to_tenant", None
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "callosum.server.tenants.billing", "register_tenant_users", None
        ...
        fetch_ee_implementation_or_noop(
            "callosum.server.tenants.user_mapping", "remove_users_from_tenant", None
        )(new_invited_emails, tenant_id)
        raise e
...
def verify_user_logged_in(
    ...
    team_name = fetch_ee_implementation_or_noop(
        "callosum.server.tenants.user_mapping", "get_tenant_id_for_email", None
    )(user.email)
    ...
    if MULTI_TENANT:
        if team_name != get_current_tenant_id():
            user_count = fetch_ee_implementation_or_noop(
                "callosum.server.tenants.user_mapping", "get_tenant_count", None
            )(team_name)
        ...
        tenant_invitation = fetch_ee_implementation_or_noop(
            "callosum.server.tenants.user_mapping", "get_tenant_invitation", None
        )(user.email)

    super_users_list = cast(
        list[str],
        fetch_versioned_implementation_with_fallback(
            "callosum.configs.app_configs",
            "SUPER_USERS",
            [],
        ),
    )
