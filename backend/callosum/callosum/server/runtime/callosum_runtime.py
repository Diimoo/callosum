...
from callosum.configs.constants import CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT
from callosum.configs.constants import CALLOSUM_CLOUD_REDIS_RUNTIME
from callosum.configs.constants import CALLOSUM_CLOUD_TENANT_ID
from callosum.configs.constants import CALLOSUM_EMAILABLE_LOGO_MAX_DIM
...
from callosum.utils.file import FileWithMimeType
from callosum.utils.file import CallosumStaticFileManager
from callosum.utils.variable_functionality import (
    fetch_ee_implementation_or_noop,
)


class CallosumRuntime:
    ...
    def _get_with_static_fallback(
        ...
        callosum_file: FileWithMimeType | None = None

        if db_filename:
            file_store = get_default_file_store()
            callosum_file = file_store.get_file_with_mime_type(db_filename)

        if not callosum_file:
            callosum_file = CallosumStaticFileManager.get_static(static_filename)
    ...
    def get_logo() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "callosum.server.enterprise_settings.store", "get_logo_filename", None
        )

        return CallosumRuntime._get_with_static_fallback(db_filename, STATIC_FILENAME)

    @staticmethod
    def get_emailable_logo() -> FileWithMimeType:
        callosum_file = CallosumRuntime.get_logo()

        # check dimensions and resize downwards if necessary or if not PNG
        image = Image.open(io.BytesIO(callosum_file.data))
        if (
            image.size[0] > CALLOSUM_EMAILABLE_LOGO_MAX_DIM
            or image.size[1] > CALLOSUM_EMAILABLE_LOGO_MAX_DIM
            or image.format != "PNG"
        ):
            image.thumbnail(
                (CALLOSUM_EMAILABLE_LOGO_MAX_DIM, CALLOSUM_EMAILABLE_LOGO_MAX_DIM),
                Image.LANCZOS,
            )  # maintains aspect ratio
            output_buffer = io.BytesIO()
            image.save(output_buffer, format="PNG")
            callosum_file = FileWithMimeType(
    ...
    def get_logotype() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "callosum.server.enterprise_settings.store", "get_logotype_filename", None
        )
    ...
    def get_beat_multiplier() -> float:
        ...
        r = get_redis_replica_client(tenant_id=CALLOSUM_CLOUD_TENANT_ID)

        beat_multiplier_raw = r.get(f"{CALLOSUM_CLOUD_REDIS_RUNTIME}:beat_multiplier")
    ...
    def get_doc_permission_sync_multiplier() -> float:
        ...
        value_raw = r.get(f"{CALLOSUM_CLOUD_REDIS_RUNTIME}:doc_permission_sync_multiplier")
    ...
    def get_build_fence_lookup_table_interval() -> int:
        ...
        interval_raw = r.get(
            f"{CALLOSUM_CLOUD_REDIS_RUNTIME}:build_fence_lookup_table_interval"
        )
