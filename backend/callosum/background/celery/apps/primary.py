...
import callosum.background.celery.apps.app_base as app_base
from callosum.background.celery.apps.app_base import task_logger
from callosum.background.celery.celery_utils import celery_is_worker_primary
from callosum.background.celery.tasks.indexing.utils import (
    get_unfenced_index_attempt_ids,
)
from callosum.configs.constants import CELERY_PRIMARY_WORKER_LOCK_TIMEOUT
from callosum.configs.constants import CallosumRedisLocks
from callosum.configs.constants import POSTGRES_CELERY_WORKER_PRIMARY_APP_NAME
from callosum.db.engine import get_session_with_default_tenant
...
from callosum.redis.redis_connector_stop import RedisConnectorStop
...
celery_app.config_from_object("callosum.background.celery.configs.primary")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    r.delete(CallosumRedisLocks.PRIMARY_WORKER)
    ...
    lock: RedisLock = r.lock(
        CallosumRedisLocks.PRIMARY_WORKER,
    ...
    r.delete(CallosumRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)
    r.delete(CallosumRedisLocks.MONITOR_VESPA_SYNC_BEAT_LOCK)
...
celery_app.autodiscover_tasks(
    [
        "callosum.background.celery.tasks.connector_deletion",
        "callosum.background.celery.tasks.indexing",
        "callosum.background.celery.tasks.periodic",
        "callosum.background.celery.tasks.doc_permission_syncing",
        "callosum.background.celery.tasks.external_group_syncing",
        "callosum.background.celery.tasks.pruning",
        "callosum.background.celery.tasks.shared",
        "callosum.background.celery.tasks.vespa",
        "callosum.background.celery.tasks.llm_model_update",
    ]
)
