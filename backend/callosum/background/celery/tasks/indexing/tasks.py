...
from callosum.background.celery.tasks.indexing.utils import _should_index
from callosum.background.celery.tasks.indexing.utils import get_unfenced_index_attempt_ids
from callosum.background.celery.tasks.indexing.utils import IndexingCallback
from callosum.background.celery.tasks.indexing.utils import try_creating_indexing_task
from callosum.background.celery.tasks.indexing.utils import validate_indexing_fences
from callosum.background.indexing.job_client import SimpleJobClient
from callosum.background.indexing.run_indexing import run_indexing_entrypoint
...
from callosum.db.connector_credential_pair import fetch_connector_credential_pairs
...
from callosum.db.index_attempt import get_last_attempt_for_cc_pair
    ...
    try:
        ...
        for cc_pair_id in cc_pair_ids:
            ...
            if tenant_id in debug_tenants:
                ttl = redis_client.ttl(CallosumRedisLocks.CHECK_INDEXING_BEAT_LOCK)
                task_logger.info(
        ...
        if not redis_client.exists(CallosumRedisSignals.VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(CallosumRedisSignals.VALIDATE_INDEXING_FENCES, 1, ex=60)
...
@shared_task(
    name=CallosumCeleryTask.CONNECTOR_INDEXING_PROXY_TASK,
    bind=True,
    acks_late=False,
    track_started=True,
)
...
@shared_task(
    name=CallosumCeleryTask.CLOUD_CHECK_FOR_INDEXING,
    trail=False,
    bind=True,
)
def cloud_check_for_indexing(self: Task) -> bool | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        CallosumRedisLocks.CLOUD_CHECK_INDEXING_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
