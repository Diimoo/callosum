...
class CallosumTracer:
    ...
    def log_previous_diff(self, numEntries: int) -> None:
        ...
        CallosumTracer.log_diff(self.snapshot, self.snapshot_prev, numEntries)

    def log_first_diff(self, numEntries: int) -> None:
        if not self.snapshot or not self.snapshot_first:
            return

        CallosumTracer.log_diff(self.snapshot, self.snapshot_first, numEntries)
