...
from callosum.configs.app_configs import WEB_CONNECTOR_ALLOWED_DOMAINS
from callosum.configs.app_configs import WEB_CONNECTOR_BLOCKED_DOMAINS
from callosum.configs.app_configs import WEB_CONNECTOR_OAUTH_CLIENT_ID
from callosum.configs.app_configs import WEB_CONNECTOR_OAUTH_CLIENT_SECRET
from callosum.configs.app_configs import WEB_CONNECTOR_OAUTH_TOKEN_URL
from callosum.configs.app_configs import WEB_CONNECTOR_VALIDATE_URLS
...
from callosum.file_processing.html_utils import web_html_cleanup
from callosum.utils.logger import setup_logger
from callosum.utils.sitemap import list_pages_for_site
...
def _enforce_domain_policy(url: str) -> None:
    ...
    if WEB_CONNECTOR_ALLOWED_DOMAINS:
        if not any(_domain_matches(hostname, domain) for domain in WEB_CONNECTOR_ALLOWED_DOMAINS):
            raise ValueError(
                f"Domain '{hostname}' is not in WEB_CONNECTOR_ALLOWED_DOMAINS, skipping page {url}."
            )

    if WEB_CONNECTOR_BLOCKED_DOMAINS:
        if any(_domain_matches(hostname, domain) for domain in WEB_CONNECTOR_BLOCKED_DOMAINS):
            raise ValueError(
                f"Domain '{hostname}' is in WEB_CONNECTOR_BLOCKED_DOMAINS, skipping page {url}."
            )
...
def protected_url_check(url: str) -> None:
    ...
    """
    # Always enforce domain allow/block policies, even if IP-level validation is disabled.
    _enforce_domain_policy(url)
...
class WebConnector(LoadConnector):
    ...
    def _fallback_fetch_with_simple_scraper(
        self,
        base_url: str,
        current_url: str,
        to_visit: list[str],
        ...
        try:
            from webscraper.web_fetcher import fetch_page  # type: ignore[import]
        except Exception as e:
            logger.warning(
                f"Fallback webscraper not available, skipping fallback for {current_url}: {e}"
            )
            return None

        try:
            res = fetch_page(current_url, respect_robots=False)
        except Exception as e:
            logger.warning(
                f"Fallback webscraper raised exception for {current_url}: {e}"
            )
            return None

        if isinstance(res, dict) and "error" in res:
            logger.info(
                f"Fallback webscraper error for {current_url}: {res.get('error')} ({res.get('reason')})"
            )
            return None

        text = cast(str, res.get("text") or "")
        title = cast(str, res.get("title") or current_url)
        html = cast(str, res.get("html") or "")

        if self.recursive and html:
        ...
        return Document(
            id=current_url,
            sections=[Section(link=current_url, text=text)],
        ...
        while to_visit:
            ...
            try:
                ...
                if page_response and str(page_response.status)[0] in ("4", "5"):
                    ...
                    fallback_doc = self._fallback_fetch_with_simple_scraper(
                        base_url, current_url, to_visit, visited_links
                    )
                    if fallback_doc is not None:
                        doc_batch.append(fallback_doc)
                    continue
...
if __name__ == "__main__":
    connector = WebConnector("https://docs.callosum.app/")
