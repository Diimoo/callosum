...
from callosum.chat.models import CallosumAnswer
from callosum.chat.models import CallosumAnswerPiece
from callosum.configs.chat_configs import QUOTE_ALLOWED_ERROR_PERCENT
from callosum.context.search.models import InferenceChunk
from callosum.prompts.constants import ANSWER_PAT
from callosum.prompts.constants import QUOTE_PAT
from callosum.utils.logger import setup_logger
from callosum.utils.text_processing import clean_model_quote
from callosum.utils.text_processing import clean_up_code_blocks
from callosum.utils.text_processing import extract_embedded_json
from callosum.utils.text_processing import shared_precompare_cleanup
...
class CallosumQuotes(BaseModel):
    quotes: list[CallosumQuote]
...
def match_quotes_to_docs(
    ...
    callosum_quotes: list[CallosumQuote] = []
    for quote in quotes:
    ...
    return CallosumQuotes(quotes=callosum_quotes)
...
def _process_answer(
...
) -> tuple[CallosumAnswer, CallosumQuotes]:
    ...
    if not answer:
        ...
        return CallosumAnswer(answer=None), CallosumQuotes(quotes=[])

    logger.notice(f"Answer: {answer}")
    if not quote_strings:
        logger.debug("No quotes extracted from raw output")
        return CallosumAnswer(answer=answer), CallosumQuotes(quotes=[])
    ...
    return CallosumAnswer(answer=answer), quotes
...
class QuotesProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[CallosumAnswerPiece | CallosumQuotes, None, None]:
        # None -> end of stream
        if token is None:
            ...
            if m:
                ...
                for quote_idx in quote_indices:
                    ...
                    if num_backslashes % 2 == 0:
                        yield CallosumAnswerPiece(answer_piece=remaining[:quote_idx])
                        return

                # If no unescaped quote found, yield the remaining string
                if len(remaining) > 0:
                    yield CallosumAnswerPiece(answer_piece=remaining)
                return
            ...
            if self.is_json_prompt and _stream_json_answer_end(model_previous, token):
                ...
                if token:
                    try:
                        ...
                        yield CallosumAnswerPiece(
                            answer_piece=self.hold_quote + token[:answer_token_section]
                        )
                    except ValueError:
                        logger.error("Quotation mark not found in token")
                        yield CallosumAnswerPiece(answer_piece=self.hold_quote + token)
                yield CallosumAnswerPiece(answer_piece=None)
                return
