...
from callosum.chat.stream_processing.utils import DocumentIdOrderMapping
from callosum.configs.chat_configs import STOP_STREAM_PAT
from callosum.prompts.constants import TRIPLE_BACKTICK
...
class CitationProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[CallosumAnswerPiece | CitationInfo, None, None]:
        # None -> end of stream
        if token is None:
            yield CallosumAnswerPiece(answer_piece=self.curr_segment)
            return
        ...
        if result:
            yield CallosumAnswerPiece(answer_piece=result)
