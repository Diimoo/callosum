...
from callosum.configs.callosumbot_configs import DANSWER_BOT_FEEDBACK_VISIBILITY
from callosum.configs.callosumbot_configs import DANSWER_BOT_MAX_QPM
from callosum.configs.callosumbot_configs import DANSWER_BOT_MAX_WAIT_TIME
from callosum.configs.callosumbot_configs import DANSWER_BOT_NUM_RETRIES
...
from callosum.connectors.slack.utils import make_slack_api_rate_limited
...
from callosum.llm.utils import dict_based_prompt_to_langchain_prompt
from callosum.llm.utils import message_to_string
...
def get_callosum_bot_slack_bot_id(web_client: WebClient) -> Any:
...
def check_message_limit() -> bool:
    ...
    if (_DANSWER_BOT_MESSAGE_COUNT + 1) > DANSWER_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            f"CallosumBot has reached the message limit {DANSWER_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD}"
...
def remove_callosum_bot_tag(message_str: str, client: WebClient) -> str:
    bot_tag_id = get_callosum_bot_slack_bot_id(web_client=client)
...
def read_slack_thread(
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            self_slack_bot_id = get_callosum_bot_slack_bot_id(client)
        ...
        message = remove_callosum_bot_tag(message, client=client)
