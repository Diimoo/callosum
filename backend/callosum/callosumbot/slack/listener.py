...
from callosum.configs.callosumbot_configs import DANSWER_BOT_REPHRASE_MESSAGE
from callosum.configs.callosumbot_configs import DANSWER_BOT_RESPOND_EVERY_CHANNEL
...
from callosum.context.search.retrieval.search_runner import download_nltk_data
from callosum.db.engine import get_all_tenant_ids
...
from callosum.callosumbot.slack.utils import get_callosum_bot_slack_bot_id
...
from callosum.callosumbot.slack.utils import respond_in_thread
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        for tenant_id in all_tenants:
            ...
            rlock = redis_client.lock(
                CallosumRedisLocks.SLACK_BOT_LOCK, timeout=TENANT_LOCK_EXPIRATION
            )
    ...
    def send_heartbeats(self) -> None:
        ...
        for tenant_id in self.tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{CallosumRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{self.pod_id}"
            redis_client.set(
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_callosum_bot_tag(msg, client=client.web_client)
        ...
        bot_tag_id = get_callosum_bot_slack_bot_id(client.web_client)
        if event_type == "message":
            is_dm = event.get("channel_type") == "im"
            is_tagged = bot_tag_id and bot_tag_id in msg
            is_callosum_bot_msg = bot_tag_id and bot_tag_id in event.get("user", "")

            # CallosumBot should never respond to itself
            if is_callosum_bot_msg:
                logger.info("Ignoring message from CallosumBot")
                return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        msg = remove_callosum_bot_tag(msg, client=client.web_client)
    ...
    try:
        with get_session_with_tenant(client.tenant_id) as db_session:
            ...
            if (
                ...
                and not is_dm
                # If /CallosumBot (is_bot_msg) or @CallosumBot (bypass_filters)
...
def _get_socket_client(
    ...
    # https://docs.callosum.app/slack_bot_setup
