...
from callosum.db.api_key import DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN
from callosum.db.models import DocumentSet__User
from callosum.db.models import Persona__User
from callosum.db.models import SamlAccount
...
def validate_user_role_update(requested_role: UserRole, current_role: UserRole) -> None:
    ...
    if current_role == UserRole.SLACK_USER:
        raise HTTPException(
            status_code=400,
            detail="To change a Slack User's role, they must first login to Callosum via the web app.",
        )
    ...
    if current_role == UserRole.EXT_PERM_USER:
        ...
        raise HTTPException(
            status_code=400,
            detail="To change an External Permissioned User's role, they must first login to Callosum via the web app.",
        )

    if current_role == UserRole.LIMITED:
        raise HTTPException(
            status_code=400,
            detail="To change a Limited User's role, they must first login to Callosum via the web app.",
        )
    ...
    if requested_role == UserRole.SLACK_USER:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "This role is automatically assigned to users who only use Callosum via Slack."
            ),
        )
