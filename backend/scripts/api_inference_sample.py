...
# In this case, the equivalent of asking a question in Callosum Chat in a new chat session
import argparse
import json
import os
...
def process_question(callosum_url: str, question: str, api_key: str | None) -> None:
    message_endpoint = callosum_url + "/api/chat/send-message"

    chat_session_id = create_new_chat_session(callosum_url, api_key)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test-question",
        type=str,
        default="What is Callosum?",
        help="Test question for new Chat Session.",
    )

    # Not needed if Auth is disabled
    # Or for Callosum MIT API key must be replaced with session cookie
    api_key = os.environ.get("DANSWER_API_KEY")

    args = parser.parse_args()
    process_question(
        callosum_url=args.callosum_url, question=args.test_question, api_key=api_key
    )
