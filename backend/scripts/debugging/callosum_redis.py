...
from callosum.configs.app_configs import REDIS_DB_NUMBER
from callosum.configs.app_configs import REDIS_HOST
from callosum.configs.app_configs import REDIS_PASSWORD
from callosum.configs.app_configs import REDIS_PORT
from callosum.configs.app_configs import REDIS_SSL
...
from callosum.redis.redis_pool import RedisPool
...
class CallosumRedisCommand(Enum):
...
def callosum_redis(
    ...
    if command == CallosumRedisCommand.purge_connectorsync_taskset:
    ...
    elif command == CallosumRedisCommand.purge_documentset_taskset:
        return purge_by_match_and_type(
            "*documentset_taskset*", "set", batch, dry_run, r
        )
    elif command == CallosumRedisCommand.purge_usergroup_taskset:
        return purge_by_match_and_type("*usergroup_taskset*", "set", batch, dry_run, r)
    elif command == CallosumRedisCommand.purge_locks_blocking_deletion:
        ...
        return 0
    elif command == CallosumRedisCommand.purge_vespa_syncing:
        return purge_by_match_and_type(
            "*connectorsync:vespa_syncing*", "string", batch, dry_run, r
        )
    elif command == CallosumRedisCommand.purge_pidbox:
        return purge_by_match_and_type(
            "*reply.celery.pidbox", "list", batch, dry_run, r
        )
    elif command == CallosumRedisCommand.get_list_element:
        ...
        print(f"{result}")
        return 0
    elif command == CallosumRedisCommand.get_user_token:
        if not user_email:
        ...
        if token_key:
            ...
            return 2
    elif command == CallosumRedisCommand.delete_user_token:
        if not user_email:
        ...
        if delete_user_token_from_redis(r, user_email, dry_run):
            ...
            return 2
    elif command == CallosumRedisCommand.add_invited_user:
        if not user_email:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Callosum Redis Manager")
    parser.add_argument(
        "--command",
        type=CallosumRedisCommand,
        help="The command to run",
        choices=list(CallosumRedisCommand),
        required=True,
    )
