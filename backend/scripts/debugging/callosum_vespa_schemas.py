"""Tool to generate all supported schema variations for Callosum Cloud's Vespa database.

Usage:

```
PYTHONPATH=. python scripts/debugging/callosum_vespa_schemas.py
```
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="callosum/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja",
    )
    parser.add_argument(
        "--cloud-services-template",
        help="The cloud-services.xml.jinja template path",
        default="ee/callosum/document_index/vespa/app_config/cloud-services.xml.jinja",
    )
    parser.add_argument(
        "--output-path",
