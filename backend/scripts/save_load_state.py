...
from callosum.document_index.vespa.index import DOCUMENT_ID_ENDPOINT
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Callosum checkpoint saving and loading.")
    parser.add_argument(
        "--save", action="store_true", help="Save Callosum state to directory."
    )
    parser.add_argument(
        "--load", action="store_true", help="Load Callosum state from save directory."
    )
    parser.add_argument(
        "--postgres_container_name",
        type=str,
        default="callosum-relational_db-1",
        help="Name of the postgres container to dump",
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default=os.path.join("..", "callosum_checkpoint"),
