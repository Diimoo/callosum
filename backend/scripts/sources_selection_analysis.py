...
from callosum.configs.app_configs import DOCUMENT_INDEX_NAME  # noqa: E402
from callosum.configs.constants import SOURCE_TYPE  # noqa: E402
...
class SelectionAnalysis:
    ...
    def do_request(self, query: str) -> dict:
        """Request the Callosum API

        Args:
            query (str): A query

        Returns:
            dict: The Callosum API response content
        """
        cookies = (
        ...
        try:
            ...
            if response.status_code != 200:
                color_output(
                    (
                        "something goes wrong while requesting the Callosum API "
            ...
            color_output(
                f"Unable to request the Callosum API for the query '{query}': {e}",
                model="critical",
            )
            sys.exit(1)
    ...
    def extract_content(self, contents: dict) -> dict:
        """Extract the content returns by the Callosum API

        Args:
            contents (dict): The callosum response content
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Currently, to get this script working when the Callosum Auth is "
    ...
    parser.add_argument(
        ...
        default=3000,
        help=(
            "The Callosum Web (not the API) port. We use the UI to forward the requests to the API. "
            "It should be '3000' for local dev and '80' if Callosum runs using docker compose."
        ),
    )
    parser.add_argument(
        "-q" "--queries",
