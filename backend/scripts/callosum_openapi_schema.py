...
from callosum.main import app as app_fn
...
def generate_client(openapi_json_path: str) -> None:
    ...
    output_dir = os.path.join(os.path.dirname(openapi_json_path), "callosum_openapi_client")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export OpenAPI schema for Callosum API (does not require starting API server)"
    )
    parser.add_argument(
