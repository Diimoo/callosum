...
from callosum.llm.models import LanguageModelInput
...
def test_user_identity_metadata_enabled(default_multi_llm: LitellmLLM) -> None:
    with (
        ...
        patch("callosum.llm.multi_llm.SEND_USER_METADATA_TO_LLM_PROVIDER", True),
    ):
        mock_response = litellm.ModelResponse(
            id="chatcmpl-123",
            choices=[
...
def test_user_identity_metadata_disabled_omits_identity(
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("callosum.llm.multi_llm.SEND_USER_METADATA_TO_LLM_PROVIDER", False),
    ):
        mock_response = litellm.ModelResponse(
            id="chatcmpl-123",
            choices=[
