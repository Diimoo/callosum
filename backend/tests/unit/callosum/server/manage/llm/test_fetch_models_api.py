...
class TestGetOllamaAvailableModels:
    ...
    def test_returns_model_list(
        ...
        from callosum.server.manage.llm.api import get_ollama_available_models

        mock_session = MagicMock()

        with patch("callosum.server.manage.llm.api.httpx") as mock_httpx:
    ...
    def test_syncs_to_db_when_provider_name_specified(
        ...
        with (
            patch("callosum.server.manage.llm.api.httpx") as mock_httpx,
            patch(
                "callosum.db.llm.fetch_existing_llm_provider", return_value=mock_provider
            ),
        ):
            mock_get_response = MagicMock()
            mock_get_response.json.return_value = mock_ollama_tags_response
...
class TestGetOpenRouterAvailableModels:
    ...
    def test_returns_model_list(self, mock_openrouter_response: dict) -> None:
        """Test that endpoint returns properly formatted model list."""
        from callosum.server.manage.llm.api import get_openrouter_available_models

        mock_session = MagicMock()

        with patch("callosum.server.manage.llm.api.httpx.get") as mock_get:
    ...
    def test_syncs_to_db_when_provider_name_specified(
        ...
        with (
            patch("callosum.server.manage.llm.api.httpx.get") as mock_get,
            patch(
                "callosum.db.llm.fetch_existing_llm_provider", return_value=mock_provider
            ),
        ):
