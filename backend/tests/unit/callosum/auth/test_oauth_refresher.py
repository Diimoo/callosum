...
from callosum.auth.oauth_refresher import _test_expire_oauth_token
from callosum.auth.oauth_refresher import check_and_refresh_oauth_tokens
from callosum.auth.oauth_refresher import check_oauth_account_has_refresh_token
from callosum.auth.oauth_refresher import get_oauth_accounts_requiring_refresh_token
from callosum.auth.oauth_refresher import refresh_oauth_token
...
async def test_refresh_oauth_token_success(
    ...
    with patch("callosum.auth.oauth_refresher.httpx.AsyncClient") as client_class_mock:
...
async def test_check_and_refresh_oauth_tokens(
    ...
    with patch(
        "callosum.auth.oauth_refresher.refresh_oauth_token", AsyncMock(return_value=True)
    ) as mock_refresh:
        # Call the function under test
