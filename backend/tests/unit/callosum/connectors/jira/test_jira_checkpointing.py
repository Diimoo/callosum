...
from callosum.connectors.jira.connector import JiraConnectorCheckpoint
from callosum.connectors.jira.utils import JIRA_SERVER_API_VERSION
...
def jira_connector(
    ...
    with patch("callosum.connectors.jira.connector._JIRA_FULL_PAGE_SIZE", 2):
        yield connector
...
def test_load_credentials(jira_connector: JiraConnector) -> None:
    ...
    with patch("callosum.connectors.jira.connector.build_jira_client") as mock_build_client:
...
def test_load_from_checkpoint_with_issue_processing_error(
    ...
    with patch("callosum.connectors.jira.connector.process_jira_issue") as mock_process:
...
def test_retrieve_all_slim_docs_perm_sync(
    ...
    with patch(
        "callosum.connectors.jira.connector.best_effort_get_field_from_issue"
    ) as mock_field:
        mock_field.side_effect = ["TEST-1", "TEST-2"]

        # Mock build_jira_url to return URLs
        with patch("callosum.connectors.jira.connector.build_jira_url") as mock_url:
