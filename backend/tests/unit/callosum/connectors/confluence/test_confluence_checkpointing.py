...
from callosum.connectors.confluence.connector import ConfluenceCheckpoint
...
from tests.unit.callosum.connectors.utils import (
...
def confluence_connector(
    confluence_base_url: str, space_key: str, mock_confluence_client: CallosumConfluence
    ...
    with patch("callosum.connectors.confluence.connector._SLIM_DOC_BATCH_SIZE", 2):
        yield connector
...
def test_load_from_checkpoint_with_page_processing_error(
    ...
    with patch(
        "callosum.connectors.confluence.connector.ConfluenceConnector._convert_page_to_document",
...
def test_validate_connector_settings_errors(
    ...
    with patch(
        "callosum.connectors.confluence.callosum_confluence.CallosumConfluence.retrieve_confluence_spaces"
    ) as mock_retrieve:
