...
from callosum.connectors.salesforce.doc_conversion import _extract_section
from callosum.connectors.salesforce.doc_conversion import ID_PREFIX
from callosum.connectors.salesforce.callosum_salesforce import CallosumSalesforce
from callosum.connectors.salesforce.salesforce_calls import _bulk_retrieve_from_salesforce
from callosum.connectors.salesforce.salesforce_calls import _make_time_filter_for_sf_type
from callosum.connectors.salesforce.salesforce_calls import _make_time_filtered_query
from callosum.connectors.salesforce.salesforce_calls import get_object_by_id_query
from callosum.connectors.salesforce.sqlite_functions import CallosumSalesforceSQLite
from callosum.connectors.salesforce.utils import ACCOUNT_OBJECT_TYPE
from callosum.connectors.salesforce.utils import MODIFIED_FIELD
from callosum.connectors.salesforce.utils import USER_OBJECT_TYPE
from callosum.utils.logger import setup_logger

# from callosum.connectors.salesforce.callosum_salesforce_type import CallosumSalesforceType
# from callosum.connectors.salesforce.salesforce_calls import get_children_of_sf_type
...
def _create_csv_file_and_update_db(
    sf_db: CallosumSalesforceSQLite,
    object_type: str,
    records: list[dict],
...
def _create_csv_with_example_data(sf_db: CallosumSalesforceSQLite) -> None:
    """
...
def _test_query(sf_db: CallosumSalesforceSQLite) -> None:
    """
...
def _test_upsert(sf_db: CallosumSalesforceSQLite) -> None:
    """
...
def _test_relationships(sf_db: CallosumSalesforceSQLite) -> None:
    """
...
def _test_account_with_children(sf_db: CallosumSalesforceSQLite) -> None:
    """
...
def _test_relationship_updates(sf_db: CallosumSalesforceSQLite) -> None:
    """
...
def _test_get_affected_parent_ids(sf_db: CallosumSalesforceSQLite) -> None:
    """
...
def test_salesforce_sqlite() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        sf_db = CallosumSalesforceSQLite(filename)
        sf_db.connect()
        sf_db.apply_schema()
...
def test_salesforce_bulk_retrieve() -> None:
    ...
    # callosum_sf_type = CallosumSalesforceType("Contact", sf_client)
...
def test_normalize_record() -> None:
    ...
    with open(current_dir / "test_account.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            normalized_record, parent_ids = CallosumSalesforceSQLite.normalize_record(row)
...
def test_salesforce_connector_single() -> None:
    ...
    # callosum_parent_sf_type = CallosumSalesforceType(parent_type, sf_client)
    ...
    for parent_type_working in parent_types:
        ...
        for child_type, child_relationship in child_types_working.items():
            # callosum_sf_type = CallosumSalesforceType(child_type, sf_client)
    ...
    normalized_record, _ = CallosumSalesforceSQLite.normalize_record(record)
