...
from callosum.connectors.web.connector import WEB_CONNECTOR_VALID_SETTINGS
...
def test_web_connector_recursive_www_redirect() -> None:
    # Check that https://callosum.app can be recursed if re-directed to www.callosum.app
    # Run in thread pool to avoid conflict with pytest-asyncio's event loop
    def _run_connector() -> list[Document]:
        connector = WebConnector(
            base_url="https://callosum.app",
