...
Then, using the VSCode/Cursor debugger, run the `Callosum OpenAPI Schema Generator` task (see `CONTRIBUTING_VSCODE.md` for `launch.json` setup instructions).
...
```sh
openapi-generator generate -i backend/generated/openapi.json -g python -o backend/generated/callosum_openapi_client --package-name callosum_openapi_client --skip-validate-spec --openapi-normalizer "SIMPLIFY_ONEOF_ANYOF=true,SET_OAS3_NULLABLE=true"
```

1. Launch callosum (using Docker or running with a debugger), ensuring the API server is running on port 8080.
   - If you'd like to set environment variables, you can do so by creating a `.env` file in the callosum/backend/tests/integration/ directory.
   - Callosum MUST be launched with AUTH_TYPE=basic and ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true
2. Navigate to `callosum/backend`.
...
callosum services with a name different from the default `callosum`.
