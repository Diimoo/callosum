...
1. Ensure you have the required dependencies installed and callosum running.

2. Ensure you have `OPENAI_API_KEY` set if you intend to do answer evaluation (enabled by default, unless you run the script with the `-s` flag). Also, if you're not using `AUTH_TYPE=disabled`, go to the API Keys page in the admin panel, generate a basic api token, and add it to the env file as `CALLOSUM_API_KEY=on_...`.

3. Navigate to Callosum repo, **search_quality** folder:

```
cd path/to/callosum/backend/tests/regression/search_quality
```
...
python run_search_eval.py
  ...
  -e --api_endpoint     # Base URL of the Callosum API server (default: http://127.0.0.1:8080)
