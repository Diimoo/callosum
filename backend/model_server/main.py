...
from model_server.speech import router as speech_router
...
def get_model_app() -> FastAPI:
    application = FastAPI(
        title="Callosum Model Server", version=__version__, lifespan=lifespan
    )
    if SENTRY_DSN:
        sentry_sdk.init(
            dsn=SENTRY_DSN,
...
if __name__ == "__main__":
    logger.notice(
        f"Starting Callosum Model Server on http://{MODEL_SERVER_ALLOWED_HOST}:{str(MODEL_SERVER_PORT)}/"
    )
