...
from ee.callosum.db.analytics import fetch_assistant_message_analytics
from ee.callosum.db.analytics import fetch_assistant_unique_users
from ee.callosum.db.analytics import fetch_assistant_unique_users_total
from ee.callosum.db.analytics import fetch_callosumbot_analytics
from ee.callosum.db.analytics import fetch_per_user_query_analytics
from ee.callosum.db.analytics import fetch_persona_message_analytics
from ee.callosum.db.analytics import fetch_persona_unique_users
from ee.callosum.db.analytics import fetch_query_analytics
from ee.callosum.db.analytics import user_can_view_assistant_stats
...
class CallosumbotAnalyticsResponse(BaseModel):
    total_queries: int
    auto_resolved: int
    date: datetime.date


@router.get("/admin/callosumbot")
def get_callosumbot_analytics(
    start: datetime.datetime | None = None,
    end: datetime.datetime | None = None,
    _: User | None = Depends(current_admin_user),
    db_session: Session = Depends(get_session),
) -> list[CallosumbotAnalyticsResponse]:
    daily_callosumbot_info = fetch_callosumbot_analytics(
        start=start
        or (
            datetime.datetime.utcnow() - datetime.timedelta(days=_DEFAULT_LOOKBACK_DAYS)
        ),  # default is 30d lookback
