...
from ee.callosum.background.task_name_builders import query_history_task_name
from ee.callosum.db.query_history import get_all_query_history_export_tasks
from ee.callosum.db.query_history import get_page_of_chat_sessions
from ee.callosum.db.query_history import get_total_filtered_chat_sessions_count
from ee.callosum.server.query_history.models import ChatSessionMinimal
from ee.callosum.server.query_history.models import ChatSessionSnapshot
from ee.callosum.server.query_history.models import MessageSnapshot
from ee.callosum.server.query_history.models import QueryHistoryExport
from callosum.auth.users import current_admin_user
from callosum.auth.users import get_display_email
from callosum.background.celery.versioned_apps.client import app as client_app
from callosum.background.task_utils import construct_query_history_report_name
from callosum.chat.chat_utils import create_chat_history_chain
from callosum.configs.app_configs import CALLOSUM_QUERY_HISTORY_TYPE
from callosum.configs.constants import FileOrigin
from callosum.configs.constants import FileType
...
from callosum.configs.constants import QueryHistoryType
from callosum.configs.constants import SessionType
...
from callosum.db.enums import TaskStatus
from callosum.db.file_record import get_query_history_export_files
from callosum.db.models import ChatSession
from callosum.db.models import User
from callosum.db.tasks import get_task_with_id
from callosum.db.tasks import register_task
...
CALLOSUM_ANONYMIZED_EMAIL = "anonymous@anonymous.invalid"


def ensure_query_history_is_enabled(
    disallowed: list[QueryHistoryType],
) -> None:
    if CALLOSUM_QUERY_HISTORY_TYPE in disallowed:
        raise HTTPException(
...
def snapshot_from_chat_session(
    ...
    flow_type = SessionType.SLACK if chat_session.callosumbot_flow else SessionType.CHAT
...
def get_chat_session_history(
    ...
    for chat_session in page_of_chat_sessions:
        ...
        if CALLOSUM_QUERY_HISTORY_TYPE == QueryHistoryType.ANONYMIZED:
            minimal_chat_session.user_email = CALLOSUM_ANONYMIZED_EMAIL
...
def get_chat_session_admin(
    ...
    if CALLOSUM_QUERY_HISTORY_TYPE == QueryHistoryType.ANONYMIZED:
        snapshot.user_email = CALLOSUM_ANONYMIZED_EMAIL
...
def start_query_history_export(
    ...
    client_app.send_task(
        CallosumCeleryTask.EXPORT_QUERY_HISTORY_TASK,
        task_id=task_id,
