...
from ee.callosum.server.enterprise_settings.models import AnalyticsScriptUpload
from ee.callosum.server.enterprise_settings.models import EnterpriseSettings
from ee.callosum.server.enterprise_settings.store import get_logo_filename
from ee.callosum.server.enterprise_settings.store import get_logotype_filename
from ee.callosum.server.enterprise_settings.store import load_analytics_script
from ee.callosum.server.enterprise_settings.store import load_settings
from ee.callosum.server.enterprise_settings.store import store_analytics_script
from ee.callosum.server.enterprise_settings.store import store_settings
from ee.callosum.server.enterprise_settings.store import upload_logo
from callosum.auth.users import current_admin_user
from callosum.auth.users import current_user_with_expired_token
from callosum.auth.users import get_user_manager
from callosum.auth.users import UserManager
...
def fetch_logo_helper(db_session: Session) -> Response:
    try:
        ...
        callosum_file = file_store.get_file_with_mime_type(get_logo_filename())
        if not callosum_file:
            raise ValueError("get_callosum_file returned None!")
    except Exception:
        ...
        return Response(content=callosum_file.data, media_type=callosum_file.mime_type)


def fetch_logotype_helper(db_session: Session) -> Response:
    try:
        file_store = get_default_file_store()
        callosum_file = file_store.get_file_with_mime_type(get_logotype_filename())
        if not callosum_file:
            raise ValueError("get_callosum_file returned None!")
    except Exception:
        raise HTTPException(
