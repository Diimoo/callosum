...
from ee.callosum.server.tenants.provisioning import setup_tenant
from ee.callosum.server.tenants.schema_management import create_schema_if_not_exists
from ee.callosum.server.tenants.schema_management import get_current_alembic_version
from callosum.background.celery.apps.app_base import task_logger
from callosum.configs.app_configs import TARGET_AVAILABLE_TENANTS
...
@shared_task(
    name=CallosumCeleryTask.CLOUD_CHECK_AVAILABLE_TENANTS,
...
def check_available_tenants(self: Task) -> None:
    ...
    r = get_redis_client(tenant_id=CALLOSUM_CLOUD_TENANT_ID)
    lock_check: RedisLock = r.lock(
        CallosumRedisLocks.CHECK_AVAILABLE_TENANTS_LOCK,
...
def pre_provision_tenant() -> None:
    ...
    lock_provision: RedisLock = r.lock(
        CallosumRedisLocks.CLOUD_PRE_PROVISION_TENANT_LOCK,
        timeout=_TENANT_PROVISIONING_SOFT_TIME_LIMIT,
    )
    ...
    try:
        ...
        if tenant_id:
            ...
            try:
                from ee.callosum.server.tenants.provisioning import (
