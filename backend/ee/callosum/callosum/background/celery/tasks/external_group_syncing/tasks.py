...
from ee.callosum.background.celery.tasks.external_group_syncing.group_sync_utils import (
...
from ee.callosum.db.connector_credential_pair import get_cc_pairs_by_source
from ee.callosum.db.external_perm import ExternalUserGroup
from ee.callosum.db.external_perm import mark_old_external_groups_as_stale
from ee.callosum.db.external_perm import remove_stale_external_groups
from ee.callosum.db.external_perm import upsert_external_groups
from ee.callosum.external_permissions.sync_params import (
...
from callosum.background.error_logging import emit_background_error
from callosum.configs.app_configs import JOB_TIMEOUT
from callosum.configs.constants import CELERY_EXTERNAL_GROUP_SYNC_LOCK_TIMEOUT
...
from callosum.db.permission_sync_attempt import complete_external_group_sync_attempt
...
from callosum.redis.redis_connector_ext_group_sync import RedisConnectorExternalGroupSync
from callosum.redis.redis_connector_ext_group_sync import (
...
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_EXTERNAL_GROUP_SYNC,
    ignore_result=True,
...
def check_for_external_group_sync(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        CallosumRedisLocks.CHECK_CONNECTOR_EXTERNAL_GROUP_SYNC_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(CallosumRedisSignals.BLOCK_VALIDATE_EXTERNAL_GROUP_SYNC_FENCES):
            ...
            r.set(
                CallosumRedisSignals.BLOCK_VALIDATE_EXTERNAL_GROUP_SYNC_FENCES,
                1,
...
def try_creating_external_group_sync_task(
    ...
    try:
        ...
        result = app.send_task(
            CallosumCeleryTask.CONNECTOR_EXTERNAL_GROUP_SYNC_GENERATOR_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair_id,
                tenant_id=tenant_id,
            ),
            queue=CallosumCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
...
@shared_task(
    name=CallosumCeleryTask.CONNECTOR_EXTERNAL_GROUP_SYNC_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_external_group_sync_generator_task(
    ...
    lock: RedisLock = r.lock(
        CallosumRedisLocks.CONNECTOR_EXTERNAL_GROUP_SYNC_LOCK_PREFIX
...
def validate_external_group_sync_fence(
    ...
    found = celery_find_task(
        payload.celery_task_id, CallosumCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC, r_celery
    )
    if found:
