...
from ee.callosum.db.connector_credential_pair import get_all_auto_sync_cc_pairs
from ee.callosum.db.document import upsert_document_external_perms
from ee.callosum.external_permissions.sync_params import get_source_perm_sync_config
...
from callosum.background.celery.celery_redis import celery_get_queued_task_ids
...
from callosum.configs.constants import CELERY_PERMISSIONS_SYNC_LOCK_TIMEOUT
from callosum.configs.constants import CELERY_TASK_WAIT_FOR_FENCE_TIMEOUT
from callosum.configs.constants import DANSWER_REDIS_FUNCTION_LOCK_PREFIX
...
from callosum.db.document import get_documents_for_connector_credential_pair_limited_columns
from callosum.db.document import upsert_document_by_connector_credential_pair
...
from callosum.db.permission_sync_attempt import complete_doc_permission_sync_attempt
from callosum.db.permission_sync_attempt import create_doc_permission_sync_attempt
from callosum.db.permission_sync_attempt import mark_doc_permission_sync_attempt_failed
...
from callosum.db.users import batch_add_ext_perm_user_if_not_exists
from callosum.db.utils import DocumentRow
from callosum.db.utils import is_retryable_sqlalchemy_error
...
from callosum.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSyncPayload
...
from callosum.server.utils import make_short_id
from callosum.utils.logger import doc_permission_sync_ctx
from callosum.utils.logger import format_error_for_logging
from callosum.utils.logger import LoggerContextVars
...
def _is_external_doc_permissions_sync_due(cc_pair: ConnectorCredentialPair) -> bool:
    ...
    source_sync_period *= int(CallosumRuntime.get_doc_permission_sync_multiplier())
...
@shared_task(
    name=CallosumCeleryTask.CHECK_FOR_DOC_PERMISSIONS_SYNC,
    ignore_result=True,
...
def check_for_doc_permissions_sync(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        CallosumRedisLocks.CHECK_CONNECTOR_DOC_PERMISSIONS_SYNC_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(CallosumRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES):
            ...
            r.set(
                CallosumRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES,
                1,
        ...
        lock_beat.reacquire()
        keys = cast(set[Any], r_replica.smembers(CallosumRedisConstants.ACTIVE_FENCES))
        for key in keys:
            key_bytes = cast(bytes, key)

            if not r.exists(key_bytes):
                r.srem(CallosumRedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
def try_creating_permissions_sync_task(
    ...
    try:
        ...
        result = app.send_task(
            CallosumCeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair_id,
                tenant_id=tenant_id,
            ),
            queue=CallosumCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
...
@shared_task(
    name=CallosumCeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_permission_sync_generator_task(
    ...
    lock: RedisLock = r.lock(
        CallosumRedisLocks.CONNECTOR_DOC_PERMISSIONS_SYNC_LOCK_PREFIX
...
def validate_permission_sync_fence(
    ...
    found = celery_find_task(
        ...
        CallosumCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        r_celery,
    )
    if found:
