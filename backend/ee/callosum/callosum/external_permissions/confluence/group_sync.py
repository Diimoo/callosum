...
from ee.callosum.external_permissions.confluence.constants import ALL_CONF_EMAILS_GROUP_NAME
from callosum.background.error_logging import emit_background_error
from callosum.configs.app_configs import CONFLUENCE_USE_CALLOSUM_USERS_FOR_GROUP_SYNC
from callosum.connectors.confluence.callosum_confluence import (
...
def _build_group_member_email_map(
    confluence_client: CallosumConfluence, cc_pair_id: int
) -> dict[str, set[str]]:
...
def _build_group_member_email_map_from_callosum_users(
...
def _build_final_group_to_member_email_map(
    confluence_client: CallosumConfluence,
    cc_pair_id: int,
    # if set, will infer confluence usernames from callosum users in addition to using the
    ...
    use_callosum_users: bool = CONFLUENCE_USE_CALLOSUM_USERS_FOR_GROUP_SYNC,
) -> dict[str, set[str]]:
    group_to_member_email_map = _build_group_member_email_map(
        confluence_client=confluence_client,
        cc_pair_id=cc_pair_id,
    )
    group_to_member_email_map_from_callosum_users = (
        (
            _build_group_member_email_map_from_callosum_users(
                confluence_client=confluence_client,
            )
        )
        if use_callosum_users
        else {}
    )

    all_group_ids = set(group_to_member_email_map.keys()) | set(
        group_to_member_email_map_from_callosum_users.keys()
    )
    final_group_to_member_email_map = {}
    for group_id in all_group_ids:
        group_member_emails = group_to_member_email_map.get(
            group_id, set()
        ) | group_to_member_email_map_from_callosum_users.get(group_id, set())
...
def confluence_group_sync(
    ...
    provider = CallosumDBCredentialsProvider(tenant_id, "confluence", cc_pair.credential_id)
    ...
    confluence_client = CallosumConfluence(is_cloud, url, provider)
