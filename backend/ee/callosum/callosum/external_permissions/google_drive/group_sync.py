...
from ee.callosum.external_permissions.google_drive.folder_retrieval import (
    get_folder_permissions_by_ids,
)
from ee.callosum.external_permissions.google_drive.folder_retrieval import (
    get_modified_folders,
)
from ee.callosum.external_permissions.google_drive.models import GoogleDrivePermission
from ee.callosum.external_permissions.google_drive.models import PermissionType
...
from callosum.connectors.google_utils.resources import AdminService
...
def _drive_folder_to_callosum_group(
    folder: FolderInfo,
    group_email_to_member_emails_map: dict[str, list[str]],
) -> ExternalUserGroup:
    """
    Converts a folder into an Callosum group.
    """
...
def _drive_member_map_to_callosum_groups(
    drive_id_to_members_map: dict[str, tuple[set[str], set[str]]],
    group_email_to_member_emails_map: dict[str, list[str]],
...
def _google_group_to_callosum_group(
    admin_service: AdminService,
    group_email: str,
) -> ExternalUserGroup:
    """
...
def _build_callosum_groups(
    drive_id_to_members_map: dict[str, tuple[set[str], set[str]]],
    group_email_to_member_emails_map: dict[str, set[str]],
    folder_info: list[FolderInfo],
) -> list[ExternalUserGroup]:
    callosum_groups: list[ExternalUserGroup] = []

    # Convert all drive member definitions to callosum groups
    ...
    # Convert all folder permissions to callosum groups
    ...
    # Convert all group member definitions to callosum groups
...
def gdrive_group_sync(
    ...
    # Each google group is an Callosum group, yield those
    group_email_to_member_emails_map: dict[str, list[str]] = {}
    for group_email in all_group_emails:
        callosum_group = _google_group_to_callosum_group(admin_service, group_email)
        group_email_to_member_emails_map[group_email] = callosum_group.user_emails
        yield callosum_group

    # Each drive is a group, yield those
    for callosum_group in _drive_member_map_to_callosum_groups(
    ...
    for folder in folder_info:
        yield _drive_folder_to_callosum_group(folder, group_email_to_member_emails_map)
