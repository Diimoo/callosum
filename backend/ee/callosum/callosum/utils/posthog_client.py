...
from ee.callosum.configs.app_configs import MARKETING_POSTHOG_API_KEY
from ee.callosum.configs.app_configs import POSTHOG_API_KEY
from ee.callosum.configs.app_configs import POSTHOG_HOST
...
# For cross referencing between cloud and www Callosum sites
...
def capture_and_sync_with_alternate_posthog(
    ...
    try:
        if cloud_user_id := props.get("callosum_cloud_user_id"):
            cloud_props = props.copy()
            cloud_props.pop("callosum_cloud_user_id", None)
...
def get_marketing_posthog_cookie_name() -> str | None:
    if not MARKETING_POSTHOG_API_KEY:
        return None
    return f"callosum_custom_ph_{MARKETING_POSTHOG_API_KEY}_posthog"
