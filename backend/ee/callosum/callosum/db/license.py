...
from ee.callosum.server.license.models import LicenseMetadata
from ee.callosum.server.license.models import LicensePayload
from ee.callosum.server.license.models import LicenseSource
from callosum.db.models import License
...
def get_used_seats(tenant_id: str | None = None) -> int:
    ...
    For self-hosted: counts all active users (includes both Callosum UI users
    and Slack users who have been converted to Callosum users).
    """
    if MULTI_TENANT:
        from ee.callosum.server.tenants.user_mapping import get_tenant_count

        return get_tenant_count(tenant_id or get_current_tenant_id())
    else:
        # Self-hosted: count all active users (Callosum + converted Slack users)
...
def update_license_cache(
    ...
    """
    from ee.callosum.utils.license import get_license_status
