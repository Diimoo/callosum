...
def fetch_callosumbot_analytics(
    start: datetime.datetime,
    end: datetime.datetime,
    db_session: Session,
    ...
    Number of CallosumBot Queries (Chat Sessions)
    Number of instances of Negative feedback OR Needing additional help
        (only counting the last feedback)
    """
    # Get every chat session in the time range which is a Callosumbot flow
    ...
    subquery_first_ai_response = (
        ...
        .where(
            ...
            ChatSession.callosumbot_flow.is_(True),
        )
        .where(
    ...
    results = (
        db_session.query(
            ...
            # feedback aren't counted against Callosumbot
            func.sum(
                case(
                    (
                        or_(
        ...
        # message of the chat session where the chat session is Callosumbot type and within the time
