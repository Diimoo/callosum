...
export CALLOSUM_BACKEND_API_HOST="${CALLOSUM_BACKEND_API_HOST:-api_server}"
export CALLOSUM_WEB_SERVER_HOST="${CALLOSUM_WEB_SERVER_HOST:-web_server}"
export CALLOSUM_MCP_SERVER_HOST="${CALLOSUM_MCP_SERVER_HOST:-mcp_server}"
...
echo "Using API server host: $CALLOSUM_BACKEND_API_HOST"
echo "Using web server host: $CALLOSUM_WEB_SERVER_HOST"
echo "Using MCP server host: $CALLOSUM_MCP_SERVER_HOST"
...
envsubst '$DOMAIN $SSL_CERT_FILE_NAME $SSL_CERT_KEY_FILE_NAME $CALLOSUM_BACKEND_API_HOST $CALLOSUM_WEB_SERVER_HOST $CALLOSUM_MCP_SERVER_HOST $NGINX_PROXY_CONNECT_TIMEOUT $NGINX_PROXY_SEND_TIMEOUT $NGINX_PROXY_READ_TIMEOUT' < "/etc/nginx/conf.d/$1" > /etc/nginx/conf.d/app.conf
...
if [ "${MCP_SERVER_ENABLED}" = "True" ] || [ "${MCP_SERVER_ENABLED}" = "true" ]; then
  ...
  envsubst '$CALLOSUM_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp_upstream.conf.inc.template" > /etc/nginx/conf.d/mcp_upstream.conf.inc
  envsubst '$CALLOSUM_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp.conf.inc.template" > /etc/nginx/conf.d/mcp.conf.inc
else
...
echo
echo "docker logs callosum-api_server-1"
echo

while true; do
  # Use curl to send a request and capture the HTTP status code
  status_code=$(curl -o /dev/null -s -w "%{http_code}\n" "http://${CALLOSUM_BACKEND_API_HOST}:8080/health")
