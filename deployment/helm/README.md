...
* cd charts/callosum
* helm install callosum . -n callosum --set postgresql.primary.persistence.enabled=false
...
* kubectl -n callosum port-forward service/callosum-nginx 8080:80
  * this will forward the local port 8080 to the installed chart for you to run tests, etc.
* When you are finished
  * helm uninstall callosum -n callosum
  * Vespa leaves behind a PVC. Delete it if you are completely done.
    * k -n callosum get pvc
    * k -n callosum delete pvc vespa-storage-da-vespa-0
  * If you didn't disable Postgres persistence earlier, you may want to delete that PVC too.

## Run as non-root user
By default, some callosum containers run as root. If you'd like to explicitly run the callosum containers as a non-root user, update the values.yaml file for the following components:
...
## Resourcing
In the helm charts, we have resource suggestions for all Callosum-owned components. 
...
1. Install and manage the KEDA operator in your cluster yourself (for example via the official KEDA Helm chart). KEDA is no longer packaged as a dependency of the Callosum chart.
